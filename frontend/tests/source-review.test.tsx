...
describe("Source Review — 검수 모드", () => {
  it("진입은 GET /applications/{aid} 1회이고 선택한 규칙의 원본 위치를 역할별 overlay로 그리며 ID를 노출하지 않는다", async () => {
    ...
    await waitFor(() => expect(dialog.querySelectorAll(".app-overlay.value")).toHaveLength(1));
    expect(dialog.querySelectorAll(".app-overlay.key")).toHaveLength(1);
    expect(dialog.querySelector(".app-overlay.value")?.getAttribute("data-range")).toBe("C3:C7");
    expect(dialog.querySelector(".app-overlay.key")?.getAttribute("data-range")).toBe("C2");
    expect(dialog.querySelectorAll(".app-overlay.unit")).toHaveLength(0);
    // 우측 패널
  ...
  it("규칙을 클릭하면 추가 API 호출 없이 우측 패널과 overlay가 바뀐다", async () => {
    const f = sourceReviewFixture();
    const { dialog } = await openReview(f);
    await waitFor(() => expect(dialog.querySelector(".app-overlay.value")?.getAttribute("data-range")).toBe("C3:C7"));
    const before = f.calls.length;
    fireEvent.click(within(screen.getByRole("group", { name: "파싱 규칙 목록" })).getByRole("button", { name: /압력/ }));
    await waitFor(() => expect(dialog.querySelector(".app-overlay.value")?.getAttribute("data-range")).toBe("D3:D7"));
    expect(dialog.querySelector(".app-overlay.key")?.getAttribute("data-range")).toBe("D2");
    const panel = within(screen.getByTestId("mapping-panel"));
  ...
  it("드래그로 다시 지정한 원본 위치는 역할별로 다음 리비전의 regions에 실린다", async () => {
    ...
    await waitFor(() => expect(dialog.querySelector(".app-overlay.value")?.getAttribute("data-range")).toBe("E3:E7"));
  ...
  it("모두 승인은 POST approve-all?wait=20 한 번으로 승인·추출하고 결과를 보여준다", async () => {
    ...
    await waitFor(() => expect(screen.getByText("승인 2/2")).toBeTruthy());
    expect(screen.getByText("발행됨", { selector: ".app-chip" })).toBeTruthy();
