...
describe("v3 작업 내역 화면", () => {
  it("요약 카드 5개·렌더 서버 상태 한 줄·작업 목록을 보여주고 진입 호출은 3개 이하다", async () => {
    ...
    expect(within(screen.getByLabelText("종류")).getAllByRole("option").map((o) => o.textContent)).toContain("마이그레이션");
    ...
    expect(within(rowOf("공정데이터_B양식 v1 · 재파싱")).getByText("진행 중 2/5", { selector: ".v3-chip" })).toBeTruthy();
    // ID 비노출(§7)
  ...
  it("카드 클릭은 ?queue=를 바꾸고 묶음 행 `대상 · 원인 · 영향 · 처리`를 보여준다; 검수 열기는 ?review=로 간다", async () => {
    ...
    expect(within(rows[0]).getByText("temperature", { selector: ".v3-chip" })).toBeTruthy();
  ...
  it("프로파일 지정은 승인된 프로파일 목록에서 고른 뒤 {assign_profile, profile_id}를 보낸다", async () => {
    ...
    expect(container.querySelector(".v3-summary")?.closest("[inert]")).toBeTruthy();
  ...
  it("묶음 행을 펼치면 멤버(문서명 · Snapshot · 상태 · 원본 보기)를 한 번만 불러온다", async () => {
    ...
    expect(within(rows[0]).getByText("검수 필요", { selector: ".v3-chip" })).toBeTruthy();
    expect(within(rows[2]).getByText("재추출 필요", { selector: ".v3-chip" })).toBeTruthy();
  ...
  it.each([
    ...
    expect(within(row).getByText("실패", { selector: ".v3-chip" })).toBeTruthy();
  ...
  it("작업 목록 필터는 URL·API에 반영되고, 진행 중 작업은 취소할 수 있다(낙관적 갱신)", async () => {
    ...
    expect(within(row).getByText("취소됨", { selector: ".v3-chip" })).toBeTruthy();
