...
import { BUILD_COLUMNS_KEY } from "../src/v3/buildModel";
import { UUID_RE, SHA_RE } from "./v3-fixture";
import { BUILD_KEY, SCHEMA_KEY, buildFixture } from "./v3-build-fixture";
...
describe("v3 데이터 빌드 화면", () => {
  ...
  it("1단계: 후보 요약·제외 사유·프로파일을 보여주고, 제거는 초안을 갱신해 후보를 다시 조회한다", async () => {
    ...
    expect(within(table).getAllByText("사용 가능", { selector: ".v3-chip" })).toHaveLength(1);
    expect(within(table).getAllByText("제외", { selector: ".v3-chip" })).toHaveLength(2);
  ...
  it("3단계 순서 변경(버튼·드래그)과 행 구성은 미리보기 요청의 컬럼 순서에 반영된다", async () => {
    ...
    expect(f.callsTo(/^\/builds\/candidates/, "POST")).toHaveLength(1);
    const order = () => Array.from(document.querySelectorAll<HTMLElement>(".v3-build-columns tbody tr")).map((tr) => tr.dataset.field);
    ...
    // 드래그: 제품명을 첫 행으로
    const rows = document.querySelectorAll<HTMLElement>(".v3-build-columns tbody tr");
  ...
  it("4단계 미리보기 셀의 원본 보기는 review·rule·sheet·range를 URL에 실어 Source Review를 연다", async () => {
    ...
    expect(container.querySelector(".v3-main")?.hasAttribute("inert")).toBe(true);
  });
  ...
  it("5단계: 형식을 고르고 생성하면 POST /builds?wait=30이 호출되고 다운로드·manifest 요약·새 빌드를 제공한다", async () => {
    ...
    expect(clicked[0].href).toContain(`/api/v3/builds/${BUILD_KEY}/download`);
