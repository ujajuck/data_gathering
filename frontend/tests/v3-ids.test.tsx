...
import { SHA_RE, UUID_RE, ids, v3Fixture } from "./v3-fixture";
...
const cases: Case[] = [
  ...
  { name: "프로파일 상세 · 기본 정보", url: `?screen=profiles&profile=${ids.profile}`, fixture: profilesFixture, landmark: () => screen.findByRole("table", { name: "적용 문서" }) },
  { name: "프로파일 상세 · 규칙", url: `?screen=profiles&profile=${ids.profile}&tab=rules`, fixture: profilesFixture, landmark: () => screen.findByRole("region", { name: "프로파일 상세" }) },
  { name: "프로파일 상세 · 필드 매핑", url: `?screen=profiles&profile=${ids.profile}&tab=mapping`, fixture: profilesFixture, landmark: () => screen.findByRole("table", { name: "필드 매핑" }) },
  { name: "프로파일 상세 · 테스트", url: `?screen=profiles&profile=${ids.profile}&tab=test`, fixture: profilesFixture, landmark: () => screen.findByRole("region", { name: "프로파일 상세" }) },
  { name: "프로파일 상세 · JSON", url: `?screen=profiles&profile=${ids.profile}&tab=json`, fixture: profilesFixture, landmark: () => screen.findByLabelText("프로파일 JSON") },
  { name: "프로파일 상세 · 변경 이력", url: `?screen=profiles&profile=${ids.profile}&tab=history`, fixture: profilesFixture, landmark: () => screen.findByRole("table", { name: "변경 이력" }) },
  ...
  { name: "설정", url: "?screen=settings", fixture: v3Fixture, landmark: () => screen.findByRole("table", { name: "정규화 프리셋 목록" }) },
...
describe("v3 화면 텍스트에 ID가 보이지 않는다(§7)", () => {
