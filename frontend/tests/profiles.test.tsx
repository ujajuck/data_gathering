...
import { PROFILE_DRAFT_KEY } from "../src/app/profileDraft";
import { UUID_RE, ids } from "./fixture";
import { GENERIC_DEFINITION, NEW_PROFILE_ID, SCHEMA_KEY, profilesFixture } from "./profiles-fixture";
...
describe("파싱 프로파일 화면", () => {
  ...
  it("'이 문서로 승인'은 헤드가 전부 승인된 행에만 보이고, 누르면 낙관적으로 승인 칩·대표 표시로 바뀌며 POST /approve를 보낸다", async () => {
    ...
    expect(within(docs).queryByText("대표", { selector: ".app-chip" })).toBeNull();
    ...
    expect(within(within(section).getByRole("table", { name: "적용 문서" })).getByText("대표", { selector: ".app-chip" })).toBeTruthy();
    expect(section.textContent).toContain("공정데이터_2024_01.xlsx r2");
  ...
  it("승인 실패(422)는 낙관적 갱신을 되돌리고 오류를 보여준다; 재파싱은 ?wait=10 작업 결과를 토스트로 알린다", async () => {
    ...
    expect(within(within(section).getByRole("table", { name: "적용 문서" })).queryByText("대표", { selector: ".app-chip" })).toBeNull();
    // 초안은 재파싱 불가
  ...
  it("필드 매핑 탭은 미지정 규칙을 강조하고, 규칙 탭은 카드와 폼으로 canonical JSON을 고쳐 PUT /profiles/{id}로 저장한다", async () => {
    ...
    expect(within(rows[1]).getByText("미지정", { selector: ".app-chip" })).toBeTruthy();
    ...
    expect(within(card).getByText("→ 온도", { selector: ".app-chip" })).toBeTruthy();
    expect(within(within(section).getByRole("article", { name: "압력" })).getByText("→ pressure", { selector: ".app-chip" })).toBeTruthy();
  ...
  it("외부 Profile Import 대화상자는 붙여넣은 JSON의 형식을 판별해 canonical 미리보기와 경고를 보여주고 POST /profiles로 저장한다", async () => {
    ...
    expect(within(preview).getByText("generic-keyvalue", { selector: ".app-chip" })).toBeTruthy();
    ...
    expect(within(preview).getByText("경고 2", { selector: ".app-chip" })).toBeTruthy();
  ...
  it("Import 대화상자의 '대표 문서로 테스트'는 canonical을 draft 저장소에 두고 ?test=draft로 이동하며, '새 프로파일'은 3.0 골격으로 시작한다", async () => {
    ...
    await f.waitForApi(/^\/profiles\/import-preview/, "POST");
    const preview = await within(dialog).findByRole("region", { name: "변환 미리보기" });
    expect(within(preview).getByText("오류 없음", { selector: ".app-chip" })).toBeTruthy();
