// 작업 내역 화면 테스트 픽스처: 공용 appFixture 위에 검수 큐(GET /queues · /queues/{kind} · members · POST actions)와
// 작업 목록(target_kind별 실패 작업 · 진행 중 작업 · 취소)을 상태 있게 덧붙인다. 묶음 처리 POST는 그 묶음을 큐에서 빼고 요약 수를 줄인다.
import type { JobResponse, QueueGroup, QueueKind, QueueMemberRow } from "../src/app/types";
import { errorBody, ids, job, page, reply, sha256, uuid, appFixture } from "./fixture";
