...
import SheetViewer from "../src/v3/SheetViewer";
import { errorBody, reply, v3Fixture } from "./v3-fixture";

const renderPath = (f: ReturnType<typeof v3Fixture>, sheet: string) => `/snapshots/${f.ids.snapshot}/sheets/${sheet}/render`;
...
describe("SheetViewer", () => {
  it("첫 창을 요청하고, 스크롤하면 보이는 창과 진행 방향 1창을 선행 요청하며 같은 창은 다시 받지 않는다", async () => {
    ...
    expect(container.querySelector(".v3-viewer")?.getAttribute("data-windows")).toBe("3");
  });
  ...
  it("셀 DOM은 3,000개를 넘지 않고 행·열 헤더를 그린다", async () => {
    ...
    await waitFor(() => expect(container.querySelectorAll(".v3-cell").length).toBeGreaterThan(0));
    const cells = container.querySelectorAll(".v3-cell").length;
    expect(cells).toBeLessThanOrEqual(3000);
    expect(cells).toBeGreaterThanOrEqual(25 * 12);
    expect(container.querySelector(".v3-col-heading")?.textContent).toBe("A");
    expect(container.querySelector(".v3-row-heading")?.textContent).toBe("1");
    // 스크롤 영역은 rendered_bounds 전체(2000행 × 200열)를 덮는다
    const size = container.querySelector<HTMLElement>(".v3-sheet-size")!;
  ...
  it("overlay는 rows[].y / columns[].x로 위치를 잡고 focus 영역으로 스크롤한다", async () => {
    ...
    const value = await waitFor(() => {
      const el = container.querySelector<HTMLElement>(".v3-overlay.value");
    ...
    expect(container.querySelector<HTMLElement>(".v3-overlay.key")?.dataset.range).toBe("A2");
    expect(container.querySelector(".v3-overlay.focus")).toBeNull();
  });
  ...
  it("review 모드에서 드래그 선택은 onSelect에 영역을 넘기고, Enter는 병합 셀 전체를 넘긴다", async () => {
    ...
    expect(container.querySelector(".v3-overlay.drag")?.getAttribute("data-range")).toBe("B2:C3");
    fireEvent.pointerUp(c3);
    expect(onSelect).toHaveBeenCalledWith({ r1: 2, c1: 2, r2: 3, c2: 3 });
    expect(container.querySelector(".v3-overlay.drag")).toBeNull();
  ...
  it("503은 '렌더 서버에 연결할 수 없음'과 다시 시도를, 4xx failed는 error.message와 retry_after 뒤 활성화되는 다시 시도를 보여준다", async () => {
    ...
    expect(container.querySelector(".v3-cell")).toBeNull();
