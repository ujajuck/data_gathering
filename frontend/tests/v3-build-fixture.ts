// 데이터 빌드 화면 테스트 픽스처: 공용 v3Fixture 위에 초안 문서, 미리보기 행(셀마다 원본 위치), 빌드 결과·manifest,
// 비동기(작업) 빌드 경로를 덧붙인다.
import type { BuildColumn, BuildManifest, BuildPreview, BuildResult, JobResponse, PreviewCell } from "../src/v3/types";
import { addToBuildDraft } from "../src/v3/buildDraft";
import { ids, job, uuid, v3Fixture } from "./v3-fixture";
...
export function buildResult(body: Row): BuildResult {
  return { build_key: BUILD_KEY, download_url: `/api/v3/builds/${BUILD_KEY}/download`, manifest: manifestFor(body) };
}
...
export function buildFixture(options: { documents?: number[]; schemaKey?: string; asyncBuild?: boolean } = {}) {
  ...
  f.overrides.set(`GET /jobs/${BUILD_JOB_ID}`, (call) => {
    ...
    const done: JobResponse = job({
      ...
      total: 3,
      label: "빌드 3개 문서",
      result: { build_key: BUILD_KEY, download_url: `/api/v3/builds/${BUILD_KEY}/download`, manifest: manifestFor(body) },
    });
    void call;
    return done;
  });
