// v3 컴포넌트 테스트 픽스처: method+path 정규식 표로 라우팅하는 fetch 목(§6 형태의 표본 데이터).
// 사용법: const f = v3Fixture(); f.renderApp("?screen=documents"); await f.waitForApi(/\/documents/);
import { vi } from "vitest";
import { render, waitFor } from "@testing-library/react";
import { createElement } from "react";
import { resetClient } from "../src/v3/client";
import { clearBuildDraft, reloadBuildDraft } from "../src/v3/buildDraft";
import type {
  ApplicationSummary,
  BuildCandidates,
  DocumentRow,
...
import { STATUS_ORDER } from "../src/v3/types";
import { parseRange, formatRange } from "../src/v3/sheetGeometry";
import Workbench from "../src/v3/Workbench";
...
export function renderWindow(sheetId: string, rangeText: string): RenderWindow {
  ...
  if (sheetId === ids.sheet1 && range.r1 <= 6 && range.c2 >= 8)
    images.push({
      ...
      url: `/api/v3/snapshots/${ids.snapshot}/render-assets/${sha256(7)}.png`,
      x: 7 * COLUMN_WIDTH,
      y: 1 * ROW_HEIGHT,
      width: 160,
      height: 72,
...
type Fixture = ReturnType<typeof v3Fixture>;
...
export function v3Fixture() {
  ...
  const routes: [string, RegExp, Handler][] = [
    ...
    ["POST", /^\/builds$/, () => ({ build_key: "abcdef0123456789", download_url: "/api/v3/builds/abcdef0123456789/download", manifest: { build_key: "abcdef0123456789", created_at: iso(0), schema: { key: SCHEMA.schema_key, rev: 3 }, sources: [], columns: [], row_mode: "record", row_count: 0, excluded: [], conflicts: [] } })],
  ];
