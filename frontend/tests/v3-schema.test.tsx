...
import { SCHEMA_KEY, schemaFixture } from "./v3-schema-fixture";
...
const treeItem = (name: string) => within(tree()).getAllByRole("treeitem").find((li) => li.querySelector(".v3-tree-name")?.textContent === name)!;

describe("v3 파싱 스키마 화면", () => {
  ...
  it("트리는 중첩 응답 1회로 전체를 그리고 접기/펼치기·키보드 이동·선택이 되며, 선택하면 ?field_key=와 필드 상세(2회 호출)가 갱신되고 폐기 필드는 흐리다", async () => {
    ...
    expect(within(treeItem("시간(초)")).getByText("폐기", { selector: ".v3-chip" })).toBeTruthy();
    ...
    expect(within(aliases).getAllByText(/온도값|Temp/, { selector: ".v3-chip" })).toHaveLength(2);
  ...
  it("그래프 보기는 layoutDomain 배치로 노드 버튼·실선(parent_of)·점선(related_to)을 그리고 확대를 바꾸며 노드 클릭이 필드를 선택한다", async () => {
    ...
    expect(svg.querySelectorAll(".v3-graph-edge.parent_of[data-from]")).toHaveLength(7);
    expect(svg.querySelectorAll(".v3-graph-edge.related_to")).toHaveLength(2);
    const dashed = svg.querySelector(".v3-graph-edge.related_to")!;
    expect(dashed.getAttribute("stroke-dasharray")).toBeTruthy();
    expect(svg.querySelector(".v3-graph-edge.parent_of[data-from]")!.getAttribute("stroke-dasharray")).toBeNull();
    // 루트 → 그룹 3개
    expect(svg.querySelectorAll(".v3-graph-edge.parent_of:not([data-from])")).toHaveLength(3);
    expect(svg.querySelectorAll(".v3-graph-hull")).toHaveLength(3);
  ...
  it("사용 프로파일·연관 문서·변경 이력 탭은 목록을 읽고, 프로파일 행은 프로파일 화면으로, 원본 보기는 Source Review로 이동한다", async () => {
    ...
    expect(within(profiles).getByText("승인", { selector: ".v3-chip" })).toBeTruthy();
    ...
    expect(within(docs).getByText("정상", { selector: ".v3-chip" })).toBeTruthy();
    expect(within(docs).getByText("검수 필요", { selector: ".v3-chip" })).toBeTruthy();
    ...
    expect(within(history).getByText("현재", { selector: ".v3-chip" }).closest("td")!.textContent).toContain("v3");
  ...
  it("필드 편집은 바뀐 항목만 PATCH /schemas/{key}/fields/{field_key}로 보내고 화면·트리를 갱신하며 실패는 알린다", async () => {
    ...
    expect(within(fieldPanel()).getByText("폐기", { selector: ".v3-chip" })).toBeTruthy();
