// v3 소스 파일을 읽는 정적 검사용 도우미(용어·import 규칙 테스트 공용).
import { existsSync, readdirSync, readFileSync, statSync } from "node:fs";
import { join, resolve } from "node:path";

// jsdom 환경에서는 import.meta.url이 http URL이므로 작업 디렉터리 기준으로 찾는다(frontend/ 또는 저장소 루트).
export const V3_ROOT = [resolve(process.cwd(), "src/v3"), resolve(process.cwd(), "frontend/src/v3")].find((d) => existsSync(join(d, "client.ts")))!;

export function listV3Files(dir = V3_ROOT): string[] {
  const out: string[] = [];
  for (const name of readdirSync(dir).sort()) {
    const full = join(dir, name);
    if (statSync(full).isDirectory()) out.push(...listV3Files(full));
    else out.push(full);
  }
  return out;
}
