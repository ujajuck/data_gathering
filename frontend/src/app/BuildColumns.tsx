...
export default function BuildColumns({
  ...
  return (
    <section className="app-card">
      ...
      <p className="app-muted">출력 Header는 필드명이 기본값입니다. 빈 값과 중복은 허용되지 않으며, 순서는 ↑↓ 버튼이나 드래그로 바꿉니다.</p>
      <div className="app-table-wrap">
        <table className="app-table app-build-columns" aria-label="출력 컬럼 설정">
          <thead>
            <tr>
              <th scope="col">사용</th>
          ...
          <tbody>
            {columns.map((c, i) => {
              ...
              const errorId = `app-build-header-error-${c.field_key}`;
              return (
                <tr
                  key={c.field_key}
                  draggable
                  data-field={c.field_key}
                  className={[c.enabled ? "" : "app-column-off", dropAt === i ? "app-drop-target" : ""].join(" ").trim() || undefined}
                ...
                >
                  ...
                  <td>
                    {c.name}
                    <span className="app-small app-muted"> {c.field_key}</span>
                  </td>
                  <td>
                    <input
                      type="text"
                    ...
                    />
                    {error && (
                      <span className="app-error app-small app-inline-error" id={errorId}>
                        {error}
                      </span>
                    )}
                  </td>
                  ...
                  <td>
                    <span className="app-inline">
                      ...
                      <span className="app-drag-handle" aria-hidden="true" title="드래그하여 순서 변경">
                        ↕
                      </span>
                    </span>
                  </td>
      ...
      {enabledCount === 0 && <p className="app-note">출력할 컬럼을 하나 이상 선택하세요.</p>}
      <div className="app-form">
        ...
        <p className="app-small app-muted">
