...
export default function SheetViewer({
  ...
  if (band) {
    for (let r = band.r1; r <= band.r2; r++) {
      ...
      if (!row) continue;
      rowHeadings.push(
        <div className="app-row-heading" key={r} style={{ top: row.y + HEADING_HEIGHT, height: row.height }}>
          {r}
        </div>,
      );
    }
    for (let c = band.c1; c <= band.c2; c++) {
      ...
      if (!col) continue;
      colHeadings.push(
        <div className="app-col-heading" key={c} style={{ left: col.x + HEADING_WIDTH, width: col.width }}>
          {columnName(c)}
        </div>,
      );
    }
  ...
  const overlayNodes = [
    ...
    .map((overlay) => {
      ...
      return geometry ? (
        <div
          key={overlay.id}
          className={"app-overlay " + overlay.kind + (overlay.id === "drag" ? " drag" : "")}
          data-kind={overlay.kind}
          data-range={formatRange(overlay)}
          style={geometry}
        >
          {overlay.label ? <span className="app-overlay-label">{overlay.label}</span> : null}
        </div>
      ) : null;
    })
    .filter(Boolean);
  ...
  const statusNode =
    status.kind === "rendering" ? (
      showFull ? (
        <div className="app-rendering" role="status">
          렌더링 중
        </div>
      ) : null
    ) : status.kind !== "idle" ? (
      <div className={"app-viewer-status " + status.kind} role="alert">
  ...
  return (
    <div className={"app-viewer " + mode} data-windows={Object.keys(windows).length}>
      {toolbar && (
        <div className="app-viewer-toolbar">
          <form
            className="app-inline"
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            ...
            {jumpError && <span className="app-error app-small">{jumpError}</span>}
          </form>
          <span>
          ...
          {mode === "review" && (
            <span className="app-legend" aria-hidden="true">
              <span>
                <i className="key" />키
              </span>
              <span>
