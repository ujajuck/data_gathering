...
export function State({
  ...
  if (resource.loading)
    return createElement("p", { className: "app-muted app-loading", role: "status" }, "불러오는 중…");
  if (resource.error)
    return createElement(
      "div",
      { className: "app-error", role: "alert" },
  ...
  if (emptyNow)
    return createElement(
      ...
      action ? createElement("div", { className: "app-empty-action" }, action) : null,
    );
  return null;
}
