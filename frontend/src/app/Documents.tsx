...
export function ProfileCell({ profiles }: { profiles: DocumentRow["profiles"] }) {
  if (!profiles?.length) return <span className="app-muted">-</span>;
  const [first, ...rest] = profiles;
  return (
    <span title={profiles.map(profileLabel).join(", ")}>
      {profileLabel(first)}
      {rest.length > 0 && <span className="app-muted"> +{rest.length}</span>}
    </span>
  );
}
...
export default function Documents() {
  ...
  // 정렬: 같은 열을 다시 누르면 방향 반전, 다른 열은 그 열의 첫 방향.
    ...
    <>
      <div inert={backgroundInert || undefined}>
        ...
        <section className="app-card">
          <div className="app-toolbar" role="search" aria-label="문서 필터">
          ...
          {items.length > 0 && (
            <div className="app-table-wrap">
              <table className="app-table" aria-label="문서 목록" aria-busy={documents.loading || undefined}>
                <thead>
                  <tr>
                    <th scope="col">
                      <input
                ...
                <tbody>
                  {items.map((doc) => (
                    ...
                    >
                      ...
                      <td>{doc.schemas?.length ? doc.schemas.map((s) => s.schema_name).join(", ") : <span className="app-muted">-</span>}</td>
                      <td>
          ...
          {selected.length > 0 && (
            <div className="app-selection-bar app-sticky-bottom" role="region" aria-label="선택한 문서">
