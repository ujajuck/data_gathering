// 파싱 스키마 > 구조 보기 > 그래프 보기(§7 SchemaGraph). GET /schemas/{key}/graph 응답을 순수 배치 함수
// layoutDomain 입력(Graph)으로 어댑트하고, SVG는 이 파일에서 그린다.
// 스키마 루트 → 그룹(level 1) → 필드. parent_of 관계는 실선, related_to는 점선. 노드는 버튼(이름 + 문서·프로파일 수).
import { useMemo, useState } from "react";
import type { KeyboardEvent as ReactKeyboardEvent } from "react";
import { GROUP_COLORS, groupColor, layoutDomain } from "./graphLayout";
import type { Graph, GraphGroup, GraphNode } from "./graphLayout";
...
export function adaptGraph(data: SchemaGraphData, schemaName: string): Graph {
  ...
  return { domain: schemaName, nodes, groups };
}
  ...
  return (
    <div className="app-graph">
      <div className="app-toolbar">
        ...
        <span className="app-legend app-small" aria-label="범례">
          <span className="app-graph-legend solid">부모 → 자식</span>
          <span className="app-graph-legend dashed">관련</span>
          <span className="app-muted">노드 {nodeCount} · 관계 {drawn.length}</span>
        </span>
      </div>
      <div className="app-graph-scroll">
        <svg
          className="app-graph-svg"
          role="img"
          aria-label="스키마 그래프"
        ...
        >
          {laid.groups.map((g) => {
            const color = groupColor(graph, g.group.root_concept_id);
            return (
              <g key={g.group.root_concept_id} className="app-graph-hull" data-group={g.group.root_concept_id}>
                <rect x={g.x} y={g.y} width={g.w} height={g.h} rx={10} fill={color} fillOpacity={0.06} stroke={color} strokeOpacity={0.35} />
                <text x={g.x + 12} y={g.y + g.h - 10} className="app-graph-hull-label" fill={color}>
          ...
          {laid.groups
            ...
            .map((g) => {
              ...
              const a = center(root);
              const b = center(to);
              return <line key={"root-" + g.group.root_concept_id} className="app-graph-edge parent_of" data-relation="parent_of" x1={a.x} y1={root.y + root.h} x2={b.x} y2={to.y} stroke={GROUP_COLORS[6]} />;
            })}
          {drawn.map((e, i) => {
            ...
            return (
              <line
                key={i}
                className={"app-graph-edge " + e.kind}
          ...
          <g className="app-graph-node root" data-level="0">
            <rect x={root.x} y={root.y} width={root.w} height={root.h} rx={7} fill="#e8f0ff" stroke="#2563eb" />
            <text x={root.x + root.w / 2} y={root.y + 21} textAnchor="middle" className="app-graph-title">
              {schemaName}
            </text>
          </g>
          {Array.from(boxes.entries()).map(([fieldKey, box]) => {
            ...
            return (
              <g
                ...
                className={"app-graph-node" + (isSelected ? " selected" : "") + (isGroup ? " group" : "")}
              ...
              >
                ...
                />
                <text x={box.x + box.w / 2} y={box.y + (isGroup ? 19 : 13)} textAnchor="middle" className={isGroup ? "app-graph-title" : "app-graph-name"}>
                  {node?.name || fieldKey}
                </text>
                {!isGroup && (
                  <text x={box.x + box.w / 2} y={box.y + 25} textAnchor="middle" className="app-graph-counts">
