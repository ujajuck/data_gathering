...
function ReviewFrame({ context, banner, children, onClose }: { context: ReactNode; banner?: ReactNode; children: ReactNode; onClose: () => void }) {
  ...
  function onKeyDown(e: ReactKeyboardEvent<HTMLDivElement>) {
    ...
    if (root.current?.querySelector(".v3-overlay.drag")) return;
    e.stopPropagation();
    onClose();
  }
  return (
    <div className="v3-overlay-screen" role="dialog" aria-modal="true" aria-label="Source Review" tabIndex={-1} ref={root} onKeyDown={onKeyDown}>
      <div className="v3-context">
...
function ZoomSelect({ value, onChange }: { value: number; onChange: (zoom: number) => void }) {
  return (
    <label className="v3-inline v3-small">
      확대
...
function SheetList({ sheets, currentId, rolesOf, onSelect }: { sheets: SheetRow[]; currentId: string | undefined; rolesOf: (sheet: SheetRow) => string[]; onSelect: (sheet: SheetRow) => void }) {
  return (
    <>
      <h3>시트</h3>
      <div className="v3-list" role="group" aria-label="시트 목록">
        {sheets.length === 0 && <p className="v3-muted v3-small">시트가 없습니다.</p>}
        {sheets.map((sheet) => {
          const roles = rolesOf(sheet);
          return (
            <button key={sheet.sheet_id} type="button" className={"v3-list-item" + (sheet.sheet_id === currentId ? " selected" : "")} aria-current={sheet.sheet_id === currentId ? "true" : undefined} onClick={() => onSelect(sheet)}>
              <span>
                <strong>{sheet.sheet_name}</strong>
                {roles.length ? <small>역할: {roles.join(", ")}</small> : <small className="v3-muted">역할 없음</small>}
              </span>
            </button>
          );
        })}
...
function RolePicker({ value, onChange }: { value: RegionRole; onChange: (role: RegionRole) => void }) {
  return (
    <span className="v3-inline v3-small">
      드래그 역할
      <span className="v3-segment" role="group" aria-label="선택 역할">
...
function ApplicationReview({ applicationId }: { applicationId: string }) {
  ...
  const context = app ? (
    <>
      ...
      <span className="v3-muted">{snapshotLabel(app.snapshot)}</span>
      <span className="v3-muted">/ {sheet?.sheet_name || "-"}</span>
      <span className="v3-muted">/ {profileLabel(app.profile)}</span>
      <span className="v3-muted">/ {schemaLabel(app.schema)}</span>
  ...
  const banner = (
    <>
      {approveAll.busy && (
        <div className="v3-note v3-context" role="status">
          모두 승인 · 추출 진행 중… {approveAll.job && `(${JOB_STATE_LABELS[approveAll.job.state]})`}
        </div>
      )}
      {approveAll.error && (
        <div className="v3-error v3-context" role="alert">
          <span>{approveAll.error}</span>
        </div>
      )}
      {approveAllResult && !approveAll.busy && (
        <div className={(approveAllResult.state === "succeeded" ? "v3-note" : "v3-error") + " v3-context"} role="status" data-testid="approve-all-result">
    ...
    <ReviewFrame context={context} banner={banner} onClose={close}>
      ...
      {app && (
        <div className="v3-split wide-right">
          <section className="v3-card tight v3-side-list" aria-label="시트와 파싱 규칙">
            ...
            <div className="v3-list" role="group" aria-label="파싱 규칙 목록">
              {mappings.length === 0 && <p className="v3-muted v3-small">규칙이 없습니다.</p>}
              {mappings.map((m) => (
                <button key={m.mapping_id} type="button" className={"v3-list-item" + (m.mapping_id === mapping?.mapping_id ? " selected" : "")} aria-current={m.mapping_id === mapping?.mapping_id ? "true" : undefined} onClick={() => go({ rule: m.rule_key, range: "" })}>
                  <span>
          ...
          {sheet ? (
            ...
            <EmptyState className="v3-card">시트가 없습니다.</EmptyState>
          )}
          <section className="v3-card tight v3-side-list" aria-label="매핑 상세">
            {mapping ? (
              <MappingPanel
...
function TestReview({ profileId, snapshotId }: { profileId: string; snapshotId: string }) {
  ...
  const context = (
    <>
      ...
      {profile.data && <span className="v3-muted">/ {schemaLabel(profile.data.schema)}</span>}
      {sheet && <span className="v3-muted">/ {sheet.sheet_name}</span>}
      <span className="v3-muted v3-small">저장하지 않는 실행입니다</span>
      <span style={{ marginLeft: "auto" }} className="v3-inline">
      ...
      <div className="v3-split wide-right">
        <section className="v3-card tight v3-side-list" aria-label="시트와 파싱 규칙">
          ...
          <div className="v3-list" role="group" aria-label="파싱 규칙 목록">
            ...
            {!test.loading && groups.length === 0 && <p className="v3-muted v3-small">결과 규칙이 없습니다.</p>}
            {groups.map((g) => (
              <button key={g.rule_key} type="button" className={"v3-list-item" + (g.rule_key === group?.rule_key ? " selected" : "")} aria-current={g.rule_key === group?.rule_key ? "true" : undefined} onClick={() => go({ rule: g.rule_key, range: "" })}>
                <span>
        ...
        {sheet && snapshotId ? (
          ...
          <EmptyState className="v3-card">{sheetsResource.loading ? "시트를 불러오는 중…" : "시트가 없습니다."}</EmptyState>
        )}
        <section className="v3-card tight v3-side-list" aria-label="테스트 결과">
          {test.loading && (
            <p className="v3-muted v3-loading" role="status">
              테스트 실행 중…
            </p>
          )}
          {test.error && (
