...
// 절대 경로가 보여도 되는 유일한 화면) + 정규화 프리셋 표 + 서버 접근 토큰(localStorage 'v3.token').
...
export default function Settings() {
  ...
  return (
    <>
      ...
      <section className="v3-card" aria-label="정규화 프리셋">
        <div className="v3-card-head">
          <h2>정규화 프리셋</h2>
          <span className="v3-muted v3-small">파싱 규칙의 value_spec.normalization에서 이름으로 참조합니다.</span>
        </div>
        <State resource={presets} empty="프리셋이 없습니다." isEmpty={!presets.loading && !presets.error && presetItems.length === 0} />
        {presetItems.length > 0 && (
          <div className="v3-table-wrap">
            <table className="v3-table" aria-label="정규화 프리셋 목록">
              <thead>
                <tr>
                  <th scope="col">이름</th>
                  <th scope="col">키</th>
...
function SettingsCards({ data, extras }: { data: SettingsResponse; extras: [string, unknown][] }) {
  ...
  return (
    <div className="v3-settings-grid">
      <section className="v3-card" aria-label="렌더 서버">
        <h2>렌더 서버</h2>
        <dl className="v3-kv">
          <dt>모드</dt>
          <dd>
      ...
      <section className="v3-card" aria-label="Reader">
        <h2>Reader</h2>
        <dl className="v3-kv">
      ...
      <section className="v3-card" aria-label="한도">
        <h2>한도</h2>
        {limits.length === 0 ? (
          <p className="v3-muted">설정된 한도가 없습니다.</p>
        ) : (
          <dl className="v3-kv">
      ...
      <section className="v3-card" aria-label="작업 공간">
        <h2>작업 공간</h2>
        <dl className="v3-kv">
          <dt>경로</dt>
          <dd>
...
// 서버 접근 토큰: 이 브라우저(localStorage 'v3.token')에만 저장된다. 저장·삭제 뒤 응답 캐시를 비운다.
function TokenCard() {
  ...
  return (
    <section className="v3-card" aria-label="서버 접근">
      <div className="v3-card-head">
        <h2>서버 접근</h2>
