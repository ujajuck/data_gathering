...
export default function TestResultPanel({
  ...
  return (
    <div className="v3-stack" data-testid="test-panel">
      <div className="v3-inline">
        ...
        <span className="v3-muted v3-small">규칙 {result.groups.length}개</span>
      </div>
      {partial && (
        <div className="v3-note" role="status" data-testid="partial-banner">
            ...
            {errors
              ...
              .map((error, i) => (
                <li key={i}>
                  ...
                  {error.message}
                  <span className="v3-muted v3-small"> ({error.code})</span>
                </li>
              ))}
          </ul>
        </div>
        ...
        <>
              ...
              {group.field ? (
                <>
                  ...
                  {group.field.unit || group.field.type ? (
                    <span className="v3-muted v3-small"> {[group.field.type, group.field.unit].filter(Boolean).join(" · ")}</span>
                  ) : null}
                </>
              ) : (
                <span className="v3-muted">미지정</span>
              )}
            </dd>
            <dt>관찰된 키</dt>
          ...
          {values.length > 0 ? (
            <div className="v3-table-wrap">
              <table className="v3-table" aria-label="테스트 값">
                <thead>
                  <tr>
                    <th scope="col">#</th>
                    <th scope="col">값</th>
