...
export function FieldFilterBar({ fieldName, onClear }: { fieldName: string; onClear: () => void }) {
  return (
    <div className="v3-toolbar" role="status" aria-label="필드 필터">
    ...
    <>
      ...
      {items.length > 0 && (
        <div className="v3-table-wrap">
          <table className="v3-table" aria-label="사용 프로파일" aria-busy={profiles.loading || undefined}>
            <thead>
              <tr>
            ...
            <tbody>
              {items.map((p) => (
                <tr key={p.profile_id} className="clickable" onClick={() => open(p.profile_id)}>
                  ...
                  <td>
                    {p.rules?.count ?? 0}
                    {p.rules?.keys?.length ? <small className="v3-muted"> · {p.rules.keys.slice(0, 5).join(", ")}</small> : null}
                  </td>
...
export function SchemaDocumentsTab({ schemaKey, fieldKey }: { schemaKey: string; fieldKey: string }) {
  ...
  return (
    <>
      ...
      {items.length > 0 && (
        <div className="v3-table-wrap">
          <table className="v3-table" aria-label="연관 문서" aria-busy={documents.loading || undefined}>
            <thead>
              <tr>
                <th scope="col">문서명</th>
  ...
  return (
    <>
      <div className="v3-toolbar">
        <span className="v3-muted v3-small v3-grow">정의 파일 리비전. 필드 편집도 새 리비전을 만듭니다.</span>
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.rev}>
                  ...
                  <td>{r.created_by || <span className="v3-muted">-</span>}</td>
                  <td className="v3-wrap">{r.summary || <span className="v3-muted">-</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
