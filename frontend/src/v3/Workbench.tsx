// v3 쉘(§7 Workbench): 사이드바 · 통합 검색 · JobBar · 라우팅(React.lazy 화면) · SourceReview 오버레이 · 토스트.
  ...
  return (
    <div className="v3">
      <div className="v3-shell">
        <aside className="v3-sidebar" inert={overlayOpen || undefined}>
          <button type="button" className="v3-brand" onClick={() => go({ screen: "documents", document: "", tab: "" })}>
            <strong>{PRODUCT_NAME}</strong>
            <small>{PRODUCT_SUBTITLE}</small>
          </button>
          <nav className="v3-nav" aria-label="주 메뉴">
            {SCREENS.map((item) => (
              <button
                key={item.id}
                type="button"
          ...
          <nav className="v3-nav v3-nav-bottom" aria-label="보조 메뉴">
            <button
              type="button"
        ...
        <header className="v3-header" inert={overlayOpen || undefined}>
          <GlobalSearch />
          <div className="v3-header-right">
            <JobBar initialRunning={status.data?.counts?.jobs_running ?? 0} />
          </div>
        </header>
        <main className="v3-main" inert={overlayOpen || undefined}>
          {status.error ? (
      ...
      {overlayOpen && !status.error && (
        <Suspense
          fallback={
            <div className="v3-overlay-screen">
              <p className="v3-muted" role="status" style={{ padding: 20 }}>
                검수 화면을 불러오는 중…
              </p>
            </div>
          }
...
function TokenPrompt({ error, retry }: { error: ApiError; retry: () => void }) {
  ...
  if (error.status !== 401 && error.status !== 403)
    return (
      <section className="v3-card v3-token-card">
...
function GlobalSearch() {
  ...
  const listId = "v3-search-results";
  return (
    <div className="v3-search" ref={box} role="search">
      <input
        type="search"
        aria-label="통합 검색"
      ...
      {open && hits && hits.q === debounced && (
        <div className="v3-search-results" id={listId} role="listbox" aria-label="검색 결과">
          {hits.error ? (
            <div className="v3-search-empty" role="alert">
              {hits.error}
            </div>
          ) : ordered.length === 0 ? (
            <div className="v3-search-empty">검색 결과가 없습니다.</div>
          ) : (
            SEARCH_ORDER.filter((kind) => ordered.some((h) => h.kind === kind)).map((kind) => (
              <div key={kind} role="group" aria-label={SEARCH_KIND_LABELS[kind]}>
                <div className="v3-search-group">{SEARCH_KIND_LABELS[kind]}</div>
                {ordered
...
export function JobBar({ initialRunning = 0 }: { initialRunning?: number }) {
  ...
  return (
    <div className="v3-jobbar" role="status">
...
function Toasts() {
  ...
  return (
    <div className="v3-toasts" role="status" aria-live="polite">
      {toasts.map((toast) => (
        <div className="v3-toast" key={toast.id}>
