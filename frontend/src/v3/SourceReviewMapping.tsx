...
export default function MappingPanel({ mapping, schemaKey, pending, onRemovePending, busy, message, onSubmit, onRollback, onShowRegion }: MappingPanelProps) {
  ...
  return (
    <div className="v3-stack" data-testid="mapping-panel" data-mapping-status={mapping.status || "none"}>
      ...
      <dl className="v3-kv">
        ...
        <dd>
          {mapping.field ? (
            ...
            <span className="v3-muted">미지정 — 승인하려면 필드를 고르세요</span>
          )}
        </dd>
        <dt>파싱 규칙</dt>
        ...
        <dd>
          {mapping.value ? (
            <>
              ...
              {mapping.value.count > 1 && <span className="v3-muted v3-small"> · 값 {mapping.value.count}개 중 첫 값</span>}
              {mapping.value.first_region && (
                <span className="v3-muted v3-small"> · {regionLabel(mapping.value.first_region.sheet_name, mapping.value.first_region.range)}</span>
              )}
            </>
          ) : (
            <span className="v3-muted">없음 (승인 후 추출)</span>
          )}
        </dd>
        <dt>원본 위치</dt>
        <dd>
          {regions.length ? (
            <ul className="v3-region-list">
              {regions.map((region) => {
        ...
        <dd>
          ...
          <span className="v3-muted v3-small"> · {originLabel(mapping.origin)} · #{mapping.revision_no}</span>
        </dd>
      </dl>

      {message && (
        <div className={message.kind === "error" ? "v3-error" : "v3-note"} role={message.kind === "error" ? "alert" : "status"}>
      ...
      {editing ? (
        ...
        <div className="v3-stack">
          {rejecting && (
            <label className="v3-stack v3-small">
              반려 사유
              <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="선택 입력" />
            </label>
          )}
          <div className="v3-inline" role="group" aria-label="검수 행동">
      ...
      <details className="v3-details">
        ...
        <pre className="v3-json">{JSON.stringify(mapping.effective_spec ?? {}, null, 2)}</pre>
      </details>
...
function MappingEditor({
  ...
  return (
    ...
    >
      ...
      <p className="v3-note v3-small">원본 위치를 바꾸려면 시트에서 역할(키·값·단위·문맥)을 고른 뒤 드래그하세요.</p>
      <label className="v3-stack v3-small">
        저장 상태
...
function RevisionHistory({ mapping, busy, onRollback }: { mapping: MappingRow; busy: boolean; onRollback: (revision: MappingRevisionRow) => Promise<boolean> }) {
  ...
  return (
    <details className="v3-details" onToggle={(e) => setOpen((e.currentTarget as HTMLDetailsElement).open)}>
              ...
              {revisions.items.map((revision) => {
                ...
                return (
                  <tr key={revision.mapping_revision_id}>
                    ...
                    <td className="v3-wrap">{revision.reason || "-"}</td>
                    <td>
                      {!isHead && (
