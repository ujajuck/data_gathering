// 문서 상세 드로어(§7, 모달: 형제 inert·focus trap·Esc — v2 DocumentDrawer 패턴만 이식). URL ?document=&tab=&sheet=.
        ...
        doc ? (
          <div className="v3-inline">
            ...
            {doc.current_snapshot ? (
              ...
              <span className="v3-muted v3-small">Snapshot 없음</span>
            )}
          </div>
        ) : (
          <h2>문서 상세</h2>
    ...
    >
      ...
      {doc && (
        <div className="v3-stack">
          ...
          <div className="v3-relation" aria-label="문서 관계">
            <span>
              <small>문서</small>
...
function SnapshotHistory({ documentId, current }: { documentId: string; current: SnapshotRef | null }) {
  ...
  return (
    <details className="v3-details" open={open}>
      <summary
        onClick={(e) => {
          e.preventDefault();
          setOpen((o) => !o);
      ...
      {open && (
        <>
          ...
          {list.length > 0 && (
            <ul className="v3-plain-list" aria-label="Snapshot 목록">
              {list.map((s) => (
                <li key={s.snapshot_id} className="v3-inline">
...
function FileTab({ snapshotId }: { snapshotId: string }) {
  ...
  return (
    <div className="v3-split two">
      <section className="v3-card tight">
        <h3>시트{list.length ? ` ${list.length}` : ""}</h3>
        <State resource={{ ...sheets, data: list }} empty="시트가 없습니다." />
        <div className="v3-list v3-side-list">
          {list.map((sheet) => (
            <button
              key={sheet.sheet_id}
              type="button"
              className={"v3-list-item" + (current?.sheet_id === sheet.sheet_id ? " selected" : "")}
    ...
    <>
      ...
      {items.length > 0 && (
        <div className="v3-table-wrap">
          <table className="v3-table" aria-label="추출 결과">
            <thead>
              <tr>
                <th scope="col">필드</th>
                ...
                <th scope="col">
                  <span className="v3-muted">행동</span>
                </th>
              </tr>
            </thead>
            <tbody>
              {items.map((v) => (
                <tr key={v.value_id}>
                  <td>
                    {v.field?.name || <span className="v3-muted">미지정</span>}
                    {v.record_key && <span className="v3-muted v3-small"> · {v.record_key}</span>}
                  </td>
...
function ProfilesTab({ doc, onReparse }: { doc: DocumentDetailData; onReparse: () => void }) {
  ...
  return (
    <>
      ...
      {rows.length > 0 && (
        <div className="v3-table-wrap">
          <table className="v3-table" aria-label="적용 프로파일" aria-busy={applications.loading || undefined}>
            <thead>
              <tr>
            ...
            <tbody>
              {rows.map((p) => (
                <tr key={p.application_id}>
                  ...
                  <td className="num">{p.heads_total !== undefined ? `${p.heads_approved ?? 0}/${p.heads_total}` : <span className="v3-muted">…</span>}</td>
                  <td>
                    {p.published === undefined ? <span className="v3-muted">…</span> : p.published ? <Chip kind="ok">발행</Chip> : <Chip kind="muted">미발행</Chip>}
                  </td>
                  <td className="v3-inline">
      ...
      {rows.length > 0 && <div className="v3-inline">{reparseButton}</div>}
    </>
  );
}
  ...
  return (
    <div className="v3-list">
      ...
      {doc.schemas.map((s) => {
        ...
        return (
          <button key={s.schema_key} type="button" className="v3-list-item" onClick={() => go({ screen: "schema", schema: s.schema_key, document: "", tab: "", sheet: "" })}>
            <span>
...
function ReparsePicker({ snapshotId, onClose, onApplied }: { snapshotId: string; onClose: () => void; onApplied: (applicationId: string | null) => void }) {
  ...
  if (job.error)
    ...
    status = (
      <span className="v3-note" role="status">
        파싱 작업이 진행 중입니다. 닫아도 상단의 진행 중 작업 표시에서 확인할 수 있습니다.
      </span>
    );
  return (
    <div className="v3-card tight" role="group" aria-label="다른 프로파일로 파싱">
      <div className="v3-toolbar">
