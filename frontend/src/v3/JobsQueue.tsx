...
export default function QueuePanel({
  ...
  return (
    <section className="v3-card" aria-label={`${QUEUE_LABELS[kind]} 큐`}>
      <div className="v3-card-head">
        <h2>{QUEUE_LABELS[kind]}</h2>
        <span className="v3-muted v3-small">같은 원인·같은 양식은 한 행으로 묶여 있습니다.</span>
      </div>
      <State resource={groups} empty={`${QUEUE_LABELS[kind]} 큐가 비어 있습니다.`} />
      {groups.items.length > 0 && (
        <div className="v3-table-wrap">
          <table className="v3-table" aria-label={`${QUEUE_LABELS[kind]} 묶음`}>
            <thead>
              <tr>
                <th scope="col">
                  <span className="v3-visually-hidden">펼치기</span>
                </th>
                <th scope="col">대상</th>
                <th scope="col">원인</th>
                <th scope="col">영향</th>
  ...
  return (
    <>
      <tr className={expanded ? "selected" : undefined}>
        ...
        <td className="v3-wrap">{group.cause}</td>
        <td>
          ...
          {names.length > 0 && (
            <div className="v3-chips">
              ...
              {names.length > 5 && <span className="v3-small v3-muted">+{names.length - 5}</span>}
            </div>
          )}
        </td>
        <td>
  ...
  return (
    <>
      ...
      {members.items.length > 0 && (
        <table className="v3-table" aria-label={`${group.label} 멤버`}>
          <thead>
            <tr>
              <th scope="col">문서명</th>
