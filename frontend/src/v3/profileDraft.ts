...
// 메모리 + sessionStorage('v3.profile.draft') — 새로 고침해도 같은 탭 안에서는 남는다.
...
export const PROFILE_DRAFT_KEY = "v3.profile.draft";
...
// sessionStorage에서 다시 읽는다(테스트·다른 탭에서 바뀐 뒤).
export function reloadProfileDraft() {
  current = load();
  emit();
}
...
export const profileDraft = {
  ...
  subscribe: subscribeProfileDraft,
};
