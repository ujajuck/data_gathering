    ...
    <section className="v3-card">
      ...
      {data && rows.length > 0 && (
        <div className="v3-table-wrap">
          <table className="v3-table v3-build-preview" aria-label="미리보기">
            <thead>
              <tr>
                <th scope="col">#</th>
                {headers.map((c) => (
    ...
    <div className="v3-stack">
      {excluded?.length > 0 && (
        <section aria-label="제외 문서">
          <h3 className="v3-small">제외 문서 {excluded.length}개</h3>
          <ul className="v3-list">
            {excluded.map((d) => (
              <li key={d.document_id} className="v3-list-item">
      ...
      {conflicts?.length > 0 && (
        <section aria-label="충돌">
          <h3 className="v3-small">충돌 {conflicts.length}건 · 첫 값을 사용합니다</h3>
          <ul className="v3-list">
            {conflicts.map((c, i) => (
              <li key={i} className="v3-list-item">
                <span>
                  {c.document_name} · {c.field_key}
                  {c.reason ? ` · ${c.reason}` : ""}
                </span>
                <span className="v3-small v3-muted">{(c.values || []).join(" / ")}</span>
              </li>
            ))}
          </ul>
        </section>
    ...
    <section className="v3-card">
      ...
      <div className="v3-form">
        <fieldset className="v3-fieldset">
          <legend>출력 형식</legend>
          <div className="v3-inline" role="radiogroup" aria-label="출력 형식">
            {FORMATS.map((f) => (
              <label key={f} className="v3-check">
                <input type="radio" name="v3-build-format" value={f} checked={format === f} disabled={runner.busy} onChange={() => setFormat(f)} />
                {FORMAT_LABELS[f]}
              </label>
            ))}
          </div>
      ...
      {runner.busy && (
        <div className="v3-inline" role="status">
          <span>
...
function ResultCard({
  ...
  return (
    <section className="v3-card tight v3-build-result" aria-label="빌드 결과">
      <div className="v3-card-head">
        <h3>
...
function ManifestSummary({ manifest, schema }: { manifest: BuildManifest; schema: SchemaRow | null }) {
  ...
  return (
    <div className="v3-stack" aria-label="manifest 요약">
      <dl className="v3-kv">
      ...
      {(manifest.columns?.length ?? 0) > 0 && (
        <div className="v3-chips" aria-label="출력 컬럼">
      ...
      {(manifest.sources?.length ?? 0) > 0 && (
        <div className="v3-table-wrap">
          <table className="v3-table" aria-label="원본 문서">
            <thead>
              <tr>
                <th scope="col">문서명</th>
