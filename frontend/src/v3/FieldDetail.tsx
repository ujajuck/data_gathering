      ...
      {data && !editing && (
        <div className="v3-stack">
          <dl className="v3-kv">
            ...
            <dt>설명</dt>
            <dd>{data.description || <span className="v3-muted">-</span>}</dd>
            <dt>타입</dt>
            <dd>{data.type || <span className="v3-muted">-</span>}</dd>
            <dt>단위</dt>
            <dd>{data.unit || <span className="v3-muted">-</span>}</dd>
            <dt>Alias</dt>
            <dd>
              {data.aliases?.length ? (
                <span className="v3-chips" aria-label="Alias">
                  {data.aliases.map((a) => (
          ...
          <h4>관계</h4>
          <dl className="v3-kv" aria-label="필드 관계">
            <dt>부모</dt>
            <dd>
          ...
          <div>
            ...
            {!newest && !values.loading && (
              <p className="v3-muted v3-small" role="note">
          ...
          {recent.length > 0 && (
            <ul className="v3-plain-list v3-recent-values" aria-label="최근 값">
              {recent.map((v) => (
                <li key={v.value_id}>
                  <div className="v3-recent-value">
                    <strong>{v.text}</strong>
                    <small className="v3-muted" title={formatDateTime(v.captured_at)}>
...
function FieldLinks({ keys, nameOf, onSelect }: { keys: string[] | null | undefined; nameOf: (key: string) => string; onSelect: (key: string) => void }) {
  if (!keys?.length) return <span className="v3-muted">없음</span>;
  return (
...
function FieldEditForm({ field, path, onCancel, onSaved }: { field: FieldDetailData; path: string; onCancel: () => void; onSaved: (next: FieldDetailData) => void }) {
  ...
  return (
    <form className="v3-form v3-stack" aria-label="필드 편집" onSubmit={submit}>
      <label>
        필드명
