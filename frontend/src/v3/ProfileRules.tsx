      ...
      {def && !editing && (
        <>
          <div className="v3-toolbar">
            <span className="v3-muted v3-small">파싱 규칙 {rules.length}개 · 시트 역할 {sheetRoleNames(def).join(", ") || "없음"}</span>
            <button type="button" className="v3-toolbar-end small primary" onClick={add} disabled={saving}>
              + 규칙 추가
            </button>
          </div>
          {rules.length === 0 && (
          ...
          <div className="v3-stack" aria-label="파싱 규칙 목록">
            {rules.map((rule) => (
              <article className="v3-card tight v3-rule-card" key={rule.rule_key} aria-label={rule.rule_name || rule.rule_key}>
...
function RuleForm({
  ...
  return (
    <form
      className="v3-card tight v3-rule-form"
    ...
    >
      ...
      <div className="v3-toolbar">
        ...
        <span className="v3-muted v3-small v3-toolbar-end">저장하면 새 리비전이 만들어집니다.</span>
      </div>
    </form>
  );
}
...
function RoleEditor({
  ...
  return (
    <fieldset className="v3-fieldset">
      ...
      <div className="v3-inline v3-small v3-muted">
