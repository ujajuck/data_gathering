...
export function Heading({
  ...
  return (
    <div className="v3-heading">
      ...
      {actions && <div className="v3-heading-actions">{actions}</div>}
    </div>
  );
}
...
export function Chip({ kind = "muted", children, title }: { kind?: ChipClass | "blue"; children: ReactNode; title?: string }) {
  return (
    <span className={"v3-chip " + kind} title={title}>
      {children}
    </span>
  );
}
...
export function Modal({
  ...
  return (
    <>
      <div className="v3-modal-backdrop" onClick={onClose} aria-hidden="true" />
      <div
        className={"v3-modal " + className}
        role="dialog"
        aria-modal="true"
        aria-label={label}
        tabIndex={-1}
      ...
      >
        <div className="v3-modal-head">
          ...
          {head}
          <button type="button" className="v3-modal-close" aria-label="닫기" onClick={onClose}>
            ×
          </button>
        </div>
        <div className="v3-modal-body">{children}</div>
        {actions && <div className="v3-modal-actions">{actions}</div>}
      </div>
    </>
  );
}
...
export function ZoomControl({ value, onChange }: { value: number; onChange: (zoom: number) => void }) {
  ...
  return (
    <span className="v3-inline" role="group" aria-label="확대">
      <button type="button" className="small" aria-label="축소" disabled={value <= 0.5} onClick={() => step(-0.1)}>
        −
      </button>
      <span className="v3-small">{Math.round(value * 100)}%</span>
...
export function EmptyState({ children, action, className = "" }: { children: ReactNode; action?: ReactNode; className?: string }) {
  return (
    <div className={className ? "v3-empty " + className : "v3-empty"}>
      <p>{children}</p>
      {action && <div className="v3-empty-action">{action}</div>}
    </div>
  );
}
