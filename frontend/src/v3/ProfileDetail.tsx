...
export default function ProfileDetail({ profileId }: { profileId: string }) {
  ...
  return (
    <section className="v3-card" aria-label="프로파일 상세">
        ...
        <>
          <div className="v3-card-head">
            <div className="v3-inline">
              ...
              <span className="v3-muted v3-small">{schemaLabel({ name: profile.schema?.name || "-" })}</span>
            </div>
            <div className="v3-inline">
              <button
                type="button"
...
function InfoTab({
  ...
  return (
    <div className="v3-stack">
      ...
      <dl className="v3-kv">
        ...
        <dd>
          {profile.reference ? (
            <span className="v3-inline">
              ...
              <span className="v3-muted v3-small">승인 {formatDateTime(profile.reference.approved_at)} · 기준 v{profile.reference.profile_rev}</span>
        ...
        <dd>
          {profile.auto_approval_active ? (
            <span>
              <Chip kind="ok">활성</Chip> <span className="v3-muted v3-small">대표 문서와 동일한 구조의 문서는 사람 개입 없이 승인·추출됩니다.</span>
            </span>
          ) : (
            <span>
      ...
      {roles.length === 0 ? (
        <p className="v3-muted">시트 역할이 없습니다.</p>
      ) : (
        <ul className="v3-list" aria-label="시트 역할">
          {roles.map(([role, spec]) => (
            <li className="v3-list-item" key={role}>
              <span>
                <strong>{role}</strong>
      ...
      {rows.length > 0 && (
        <>
          {candidates.length === 0 && profile.status !== "approved" && (
            <p className="v3-note">승인 후보가 없습니다. Source Review에서 매핑을 모두 승인한 문서가 있어야 '이 문서로 승인'할 수 있습니다.</p>
          )}
          <div className="v3-table-wrap">
            <table className="v3-table" aria-label="적용 문서">
              <thead>
                <tr>
                  <th scope="col">문서명</th>
...
function MappingTab({ profile }: { profile: ProfileDetailData }) {
  ...
  return (
    <div className="v3-stack">
      ...
      <div className="v3-table-wrap">
        <table className="v3-table" aria-label="필드 매핑">
          <thead>
            <tr>
          ...
          <tbody>
            {profile.rules.map((rule) => (
              <tr key={rule.rule_key} className={rule.field ? undefined : "v3-unmapped"} data-unmapped={rule.field ? undefined : "true"}>
                <td>
...
function TestTab({ profileId, documents }: { profileId: string; documents: PageResource<ProfileDocumentRow> }) {
  ...
  const option = (doc: DocumentChoice, hint: string) => (
    <label className="v3-check v3-list-item" key={doc.document_id}>
      <input
        type="radio"
        name="test-document"
  ...
  return (
    <div className="v3-stack">
      <p className="v3-muted v3-small">문서를 고르고 테스트를 실행하면 저장 없이 현재 리비전을 적용한 결과를 Source Review에서 확인합니다.</p>
      ...
      {applied.length > 0 && (
        <div className="v3-list" role="radiogroup" aria-label="적용 문서">
      ...
      {q && (
        <>
          ...
          {found.length > 0 && (
            <div className="v3-list" role="radiogroup" aria-label="검색 결과">
    ...
    <div className="v3-stack">
      ...
      {definition.data && (
        <>
          ...
          {preview && (
            <div className="v3-stack v3-problems" aria-label="검증 결과">
          ...
          <div className="v3-toolbar">
            ...
            <span className="v3-toolbar-end v3-inline">
              <label>
                테스트 문서
  ...
  return (
    <div className="v3-stack">
      ...
      {revisions.items.length > 0 && (
        <div className="v3-table-wrap">
          <table className="v3-table" aria-label="변경 이력">
            <thead>
              <tr>
                <th scope="col">리비전</th>
                <th scope="col">시각</th>
            ...
            <tbody>
              {revisions.items.map((row) => (
                <tr key={row.rev}>
                  ...
                  <td className="v3-wrap">{row.summary || row.format_detected || "-"}</td>
                  <td>
