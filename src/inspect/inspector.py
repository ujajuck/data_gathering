...
def _is_drm_file(path: Path) -> bool:
    """벤더 DRM 컨테이너 감지 — 제품 시그니처는 저장소에 두지 않고
    KG_DRM_MAGIC 환경변수(콤마 구분 선두 바이트 문자열)로 주입한다.
    (src는 Parser library — kg에 의존하지 않으므로 파싱을 자체 구현)"""
    import os
    magics = [m.encode("utf-8") for m in
              os.environ.get("KG_DRM_MAGIC", "").split(",") if m]
    if not magics:
        return False
    try:
