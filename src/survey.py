...
2. 모호한 라벨 (후보는 있으나 auto 임계 미달 → 동의어 승인 필요)
...
from src.segment.detector import _FALLBACK_UNITS, segment_workbook
...
class VocabularySurveyor:
    ...
    def survey(self, paths: list[Path]) -> dict:
        ...
        for path in sorted(Path(p) for p in paths):
            ...
            rel = self.repo_root if path.resolve().is_relative_to(
                self.repo_root.resolve()) else None
            ...
            # 결정(필드 시그니처 단위) → 라벨별 최고 후보/판정 집계용 색인
            ...
            for rec in records:
                for o in rec.observations:
                    ...
                    st = labels.setdefault(key, {
                        "label": o.raw_label, "observations": 0,
                    ...
                    for tok in {o.raw_label, *o.header_path}:
                        if self._suspected_unit_token(tok):
                            ...
                            uu["observations"] += 1
                            uu["labels"].add(f"{o.source_sheet}: {o.raw_label}")
                    ...
                    if v is not None and len(st["sample_values"]) < _SAMPLE_LIMIT:
                        sv = str(v)[:_VALUE_TRUNC]
                        if sv not in st["sample_values"]:
                            st["sample_values"].append(sv)
                    # 이 라벨의 최고 후보 (pending이어도 후보 개념은 실려 온다)
                    for d in dec_by_label.get(key, []):
                        if d.concept_id and d.confidence > st["confidence"]:
        ...
        return self._build_report(files, labels, units_unknown, total_obs, total_mapped)

    # ------------------------------------------------------------ report ----
    def _build_report(self, files, labels, units_unknown, total_obs, total_mapped) -> dict:
        unknown: list[dict] = []
        ambiguous: list[dict] = []
    ...
    def _proposal_yaml(self, unknown, ambiguous, unknown_units) -> str:
        ...
        if unknown:
            ...
            for r in unknown:
                units = f", 단위 {'/'.join(r['units'])}" if r["units"] else ""
                lines.append(f"- label: {r['label']!r}"
                             f"    # 관측 {r['observations']}건{units}, "
                             f"예: {', '.join(r['sample_values'][:2])}")
        if unknown_units:
            lines.append("new_units:  # units.yaml dimensions/aliases에 등록 필요")
            for u in unknown_units:
                lines.append(f"- unit: {u['unit']!r}"
...
def survey_dir(repo_root: Path, raw_dir: Path) -> dict:
    paths = [p for p in sorted(Path(raw_dir).glob("*.xlsx")) if not p.name.startswith("~$")]
