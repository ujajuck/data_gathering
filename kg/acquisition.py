...
컨테이너 판별:
  ...
  vendor_drm: 배포처별 시그니처 — 사내/벤더 DRM, Excel COM으로 읽기 가능.
              제품 식별자를 저장소에 두지 않기 위해 시그니처는 환경변수
              KG_DRM_MAGIC(콤마 구분, 각 항목은 파일 선두 바이트의 문자열
              표기)으로 주입한다. 미설정이면 vendor_drm 판별은 꺼진다.
  ole_cfb   : D0 CF 11 E0    — 표준 오피스 암호화(비밀번호), MIP/AIP 라벨,
                               구형 .xls, 일부 DRM의 CFB 래퍼
...
_CONTAINER_LABEL = {
    "vendor_drm": "사내 DRM — Excel COM 자동 판독 가능 (Windows 전용)",
    "ole_cfb": "OLE/CFB — 암호화된 Office 문서 또는 DRM 래퍼",
    "unknown": "알 수 없는 컨테이너 — 벤더 전용 DRM 추정",
}


def vendor_magics() -> list[bytes]:
    """운영 환경이 주입한 벤더 DRM 시그니처 목록 (KG_DRM_MAGIC).

    매 호출 시 읽는다 — 테스트가 monkeypatch.setenv로 주입할 수 있게.
    """
    raw = os.environ.get("KG_DRM_MAGIC", "")
    return [m.encode("utf-8") for m in raw.split(",") if m]


def is_vendor_locked(head: bytes) -> bool:
    return any(head.startswith(m) for m in vendor_magics())
...
def sniff_container(path: Path) -> dict:
    ...
    if is_vendor_locked(head):
        return {"container": "vendor_drm", "locked": True, "com_readable": True,
                "detail": _CONTAINER_LABEL["vendor_drm"]}
