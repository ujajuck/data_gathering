...
def cmd_watch(ws: Workspace, args) -> int:
    """raw 디렉터리 폴링 → 새/변경 파일 자동 ingest(+map) — 현행 파이프라인의
    watcher (src.cli watch의 이관). 잠긴(DRM) 파일은 건드리지 않고 해제본
    도착 감지(acquisition)에 맡긴다. 삭제 이벤트는 무시한다 — 문서 제거는
    명시적 작업이지 파일 사라짐의 부수효과가 아니다."""
    from kg.acquisition import refresh_release_states, sniff_container
    ...
    def handle(ev) -> None:
        if ev.kind == "deleted":
            return
        path = Path(ev.path)
        if sniff_container(path).get("locked"):
            print(f"watch: {path.name} 잠김(DRM/암호화) — 해제 요청 흐름으로 처리",
                  file=sys.stderr)
            return
        try:
            result = _ingest_file(ws, path, force=False)
            print(json.dumps({"event": ev.kind, **result},
                             ensure_ascii=False, default=str))
            if args.map:
                cmd_map(ws, args)
        except Exception as e:                        # 한 파일 실패가 루프를 죽이지 않게
            print(f"watch: {path.name} 실패 — {e!r}", file=sys.stderr)

    import time
    loops = 0
    while True:
        refresh_release_states(ws.store, raw)         # 해제본 도착 자동 감지
        ws.store.commit()
        for ev in watcher.scan_once():
            handle(ev)
        loops += 1
        if args.scans and loops >= args.scans:        # 0 = 무한 루프
            return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    pw = sub.add_parser("watch", help="raw 폴링 → 자동 ingest(+map) — 현행 watcher")
    pw.add_argument("--raw", required=True)
    pw.add_argument("--interval", type=float, default=2.0)
    pw.add_argument("--scans", type=int, default=0,
                    help="N회 스캔 후 종료 (0=무한, 테스트/원샷용)")
