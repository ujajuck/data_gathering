...
def parse_range(value):
    """`kg.v2.spec.bounds`로 파싱하되 v3 Problem으로 바꾼다(v2 Problem은 v3 API가 잡지 못한다)."""
    if value in (None, ""):
        value = DEFAULT_RANGE
    try:
        return v2_bounds(value)
...
class RenderCache:
    def __init__(self, root, renderer_version=RENDERER_VERSION):
        ...
        self.lru_limit = int(os.environ.get("KG_V3_RENDER_LRU_MB", "256")) * 1024 * 1024
        self.disk_limit = int(os.environ.get("KG_V3_RENDER_CACHE_MB", "4096")) * 1024 * 1024
