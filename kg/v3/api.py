"""v3 API(계약 §6, prefix `/api/v3`) — 서비스 규칙은 `service.py`에 있고 여기는 HTTP 경계만 맡는다.

공통: 오류 `{error:{code,message,fields?}}`, 목록 `{items,has_more,next_cursor}`(keyset, limit ≤ 200), 본문 2MB, 접근 토큰
`KG_V3_ACCESS_TOKEN`(없으면 `KG_V2_ACCESS_TOKEN`), `Cache-Control: no-store`(렌더 창·asset만 `private, no-cache` + ETag/304),
