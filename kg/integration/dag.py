"""Transformation DAG 엔진 — §10의 전처리 블록 (13종 + value_normalize).

블록은 Frame 목록을 받아 Frame 목록을 돌려준다. 프로젝트 정의의 transform
목록(선형 DAG)을 순서대로 적용하며, 각 셀의 lineage(payload/행/노드)를
변환을 통과시키며 보존한다 — 통합 결과의 모든 값은 원본 셀까지 역추적된다 (§11).

소유권: run_dag는 입력 Frame을 소유하고 제자리 수정한다(블록별 deepcopy 없음
— 수십만 셀 규모의 지배 비용이었다). 입력 보존이 필요하면 호출 전에 clone().
"""
...
def _each_frame(fn):
    """블록 실행 래퍼 — 소유권 규약: run_dag는 입력 Frame을 소유하며 제자리
    수정한다. 블록마다 deepcopy(clone)하면 수십만 셀 규모에서 그 복사가
    지배 비용이 된다(문서 수백 개 병목의 ⅓). 입력을 보존해야 하는 호출자는
    run_dag 전에 직접 clone() 하라."""
    def wrapper(frames: list[Frame], config: dict, env: dict) -> list[Frame]:
        return [fn(f, config or {}, env) for f in frames]
    return wrapper
...
def _unit_convert(f: Frame, cfg: dict, env: dict) -> Frame:
    ...
    if units is None:
        return f
    # 변환은 전부 아핀(v*a+b) — (src,tgt) 쌍마다 계수를 한 번만 구해 두면
    # 셀 단위 units.convert() 호출(단위 정규화 포함)을 곱셈-덧셈으로 줄인다.
    conv: dict[tuple, tuple | None] = env.setdefault("_affine_cache", {})

    def coeffs(src: str, tgt: str):
        key = (src, tgt)
        if key not in conv:
            try:
                b = units.convert(0.0, src, tgt)
                conv[key] = (units.convert(1.0, src, tgt) - b, b)
    ...
    for i, r in enumerate(f.rows):
        for col, tgt in targets.items():
            ...
            if not src_unit or src_unit == tgt:
                continue
            ab = coeffs(src_unit, tgt)
            if ab is not None:
                r[col] = round(float(v) * ab[0] + ab[1], 6)
