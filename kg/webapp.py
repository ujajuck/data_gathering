...
def _is_drm_file(path: Path) -> bool:
    """벤더 DRM 컨테이너 감지 — 시그니처는 KG_DRM_MAGIC 환경변수 주입."""
    from kg.acquisition import is_vendor_locked
    try:
        with open(path, 'rb') as f:
            return is_vendor_locked(f.read(16))
    except Exception:
        return False
...
def _render_sheet_drm(path: Path, sheet_name: str | None) -> dict:
    """벤더 DRM 파일 렌더 — Excel COM 경유 (Windows + Excel 필요).
...
def create_app(ws_root: str | Path) -> FastAPI:
    ...
    def proposal(body: ProposalReq):
        ...
        with lock:
            ...
            docs: set[str] = set()
            # 캡 없음 — 트리 일괄 선택으로 수천 위치가 올 수 있다. 노드당
            # 개별 조회(3쿼리) 대신 청크 JOIN으로 한 번에 읽는다. 요청된
            # 위치는 전부 처리되거나 stale로 명시된다 — 침묵 누락 금지.
            seen: dict[str, bool] = {}      # node_id → 유효 처리 여부
            for i in range(0, len(body.node_ids), 500):
                chunk = body.node_ids[i:i + 500]
                rows = store.conn.execute(
                    f"""SELECT n.node_id, n.status n_status, n.unit n_unit,
                               n.data_type n_dtype, n.metadata, d.filename,
                               m.concept_id m_concept, m.status m_status,
                               c.canonical_name, c.canonical_name_en,
                               c.canonical_unit, c.data_type c_dtype,
                               c.concept_type
                          FROM tree_node n
                          LEFT JOIN semantic_mapping m
                            ON m.tree_node_id=n.node_id AND m.is_active=1
                          LEFT JOIN domain_concept c ON c.concept_id=m.concept_id
                          LEFT JOIN document d ON d.document_id=n.document_id
                         WHERE n.node_id IN ({",".join("?" * len(chunk))})""",
                    chunk).fetchall()
                for r in rows:
                    nid = r["node_id"]
                    if seen.get(nid):        # 중복 활성 매핑 방어 — 첫 행만
                        continue
                    if r["n_status"] != "ACTIVE" or not r["m_concept"] \
                            or r["canonical_name"] is None:
                        seen.setdefault(nid, False)
                        continue
                    seen[nid] = True
                    usable = r["m_status"] in ("AUTO_APPROVED", "APPROVED")
                    g = by_concept.setdefault(r["m_concept"], {
                        "concept_id": r["m_concept"],
                        "concept_name": r["canonical_name"],
                        "field_name": re.sub(
                            r"[^A-Za-z0-9_]", "_",
                            (r["canonical_name_en"] or r["m_concept"])
                            .strip().lower().replace(" ", "_")) or r["m_concept"],
                        "target_unit": r["canonical_unit"], "type": r["c_dtype"],
                    ...
                    if usable:
                        ...
                        g["review"] += 1
                    if r["n_unit"]:
                        g["units"].add(r["n_unit"])
                    if g["role"] is None:
                        meta = json.loads(r["metadata"] or "{}")
                        g["role"] = _node_role(meta, r["n_dtype"], r["concept_type"])
                    if r["filename"]:
                        docs.add(r["filename"])
            # 재적재로 사라졌거나 매핑 해제된 위치 — 조용히 버리지 않고 알린다
            stale = [nid for nid in body.node_ids if not seen.get(nid)]
        fields = []
    ...
    def build_db(body: BuildReq):
        ...
        from kg.integration.builder import (define_project, execute_dag,
                                            finalize_build, find_reusable_build,
                                            mark_build_failed, prepare_build,
                                            serialize_result)
        ...
        # 3단계 빌드 — 변환 DAG(순수 CPU, 대형 묶음에서 수십 초)를 도는 동안
        # 전역 락을 잡지 않는다. DB를 만지는 조립/기록 구간만 락 안이다.
        iid = None
        prep = None
        try:
            with lock:
                # 증분: 구성·선택·원본(payload)이 그대로면 이전 산출물을 즉시
                # 재사용한다 — "안 바꿨으면 즉시 반환". 재계산·새 버전 없음.
                reused = find_reusable_build(
                    store, config, getattr(units, "version", "") or "")
            if reused is not None:
                ...
                with wlock():
                    ...
                    prep = prepare_build(store, iid, units)
                # 변환 + 직렬화는 순수 CPU — 락 밖에서 돈다. 서버는 계속 응답.
                result_frames = execute_dag(prep)
                prep["_serialized"] = serialize_result(prep, result_frames)
                with wlock():
                    result = finalize_build(store, prep, result_frames,
                                            root / "data" / "kg" / "builds")
        ...
        return {
            "status": status, "reused": bool(result.get("reused")),
