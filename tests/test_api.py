...
from schema.render.client import RenderClient, RenderUnavailable
from schema.render.renderer import render_events
...
class Env:
    ...
    def get(self, path, expect=200, **kw):
        response = self.client.get("/api" + path, **kw)
    ...
    def post(self, path, body=None, expect=200, **kw):
        response = self.client.post("/api" + path, json=body, **kw)
        assert response.status_code == expect, (path, response.status_code, response.text[:400])
...
def test_render_proxy_202_200_304_and_assets(world):
    ...
    path = f"/api/snapshots/{sid}/sheets/{sheets[demo.MAIN_SHEET]}/render"
    ...
    image_path = f"/api/snapshots/{sid}/sheets/{sheets[demo.IMAGE_SHEET]}/render"
    ...
    assert len(images) == 1 and images[0]["url"].startswith(f"/api/snapshots/{sid}/render-assets/")
    ...
    assert world.client.get(f"/api/snapshots/{sid}/render-assets/..%2F..%2Fetc%2Fpasswd").status_code == 404
    unknown = world.client.get("/api/snapshots/etc/passwd")
    ...
    assert world.client.get(f"/api/snapshots/{sid}/render-assets/{'0' * 64}.png").status_code == 404
    assert world.client.get(f"/api/snapshots/{sid}/render-assets/{images[0]['asset_id']}.exe").status_code == 404
    other = world.snapshot_id(demo.OTHER_DOCUMENT)
    assert world.client.get(f"/api/snapshots/{other}/render-assets/{images[0]['asset_id']}").status_code == 404  # asset은 snapshot 안에서만
...
def test_profiles_list_detail_revisions_export(world):
    ...
    export = world.client.get(f"/api/profiles/{world.profile_id}/export")
...
def test_profile_import_preview_create_update_and_test(world):
    ...
    response = world.client.put(f"/api/profiles/{created['profile_id']}", json={"definition": moved})
    assert response.status_code == 422 and response.json()["error"]["code"] == "SCHEMA_IMMUTABLE"
    updated = world.client.put(f"/api/profiles/{created['profile_id']}", json={"definition": V2_TEMPLATE, "name": "v1 이관(v2 정의)"}).json()
...
def test_body_limit_and_validation_shapes(world):
    big = world.client.post("/api/profiles/import-preview", content=b'{"schema_key":"x","definition":{"pad":"' + b"a" * (2 * 1024 * 1024 + 10) + b'"}}', headers={"content-type": "application/json"})
