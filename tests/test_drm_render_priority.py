...
# 벤더 DRM 시그니처는 저장소에 없다 — 테스트가 KG_DRM_MAGIC으로 주입한다
_DRM_MAGIC = "#TESTDRM"
_LOCKED = _DRM_MAGIC.encode() + b"\x00" * 128


@pytest.fixture()
def ws_client(tmp_path, monkeypatch):
    monkeypatch.setenv("KG_DRM_MAGIC", _DRM_MAGIC)
    wsdir = tmp_path / "ws"
...
def test_drm_locked_falls_back_with_degraded_notice(ws_client):
    ...
    (wsdir / "data" / "raw" / "mini.xlsx").write_bytes(_LOCKED)
    data = c.get(f"/api/sheet?doc={doc}").json()
