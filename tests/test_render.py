...
from schema.render import RENDERER_VERSION
from schema.render.assemble import assemble
from schema.render.cache import RenderCache
from schema.render.client import RenderClient, RenderUnavailable
from schema.render.renderer import BAND_BYTES, _split_band, render_events
...
def test_truncation_caps(root, monkeypatch):
    ...
    monkeypatch.setattr("schema.render.renderer.MAX_CELLS", 150)
...
def test_assemble_files_and_window_invariants(root, cache):
    ...
    assert meta["images"][0]["url"] == f"/api/snapshots/{SNAPSHOT_A}/render-assets/{meta['images'][0]['asset_id']}"
...
def test_render_client_inprocess_mode(root, monkeypatch):
    monkeypatch.delenv("SCHEMA_RENDER_URL", raising=False)
...
def test_render_client_unreachable(root, monkeypatch):
    monkeypatch.setenv("SCHEMA_RENDER_URL", "http://127.0.0.1:9")
