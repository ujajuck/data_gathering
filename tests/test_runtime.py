...
Reader 호출은 `schema.service.reader_result/reader_events`를 감싸 연산 이름을 기록한다(격리 프로세스는 실제로 뜬다)."""
...
from schema.db import now
from schema.render.client import RenderClient
from tests.test_api import inprocess_source
...
def test_builds_candidates_preview_three_formats_manifest_and_header_validation(world):
    ...
    for fmt in ("csv", "xlsx", "sqlite"):
        ...
        assert result["build_key"] == preview["build_key"] and result["download_url"] == f"/api/builds/{result['build_key']}/download?format={fmt}"
    ...
    download = world.client.get(f"/api/builds/{key}/download?format=csv")
    ...
    xlsx = world.client.get(f"/api/builds/{key}/download?format=xlsx")
    ...
    sqlite_file = world.client.get(f"/api/builds/{key}/download?format=sqlite")
...
def test_document_list_page_under_50ms_with_2000_documents(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        first = client.get("/api/documents?limit=50").json()
        ...
        paths = [
            ...
            f"/api/documents?limit=50&cursor={first['next_cursor']}",
            "/api/documents?limit=50&status=review",
            "/api/documents?limit=50&sort=document_name",
            "/api/documents?limit=50&q=문서_19",
        ]
        ...
        assert client.get(f"/api/documents?limit=50&status=review").json()["items"][0]["status"] == "review"
