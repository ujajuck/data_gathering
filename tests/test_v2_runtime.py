...
def test_graph_is_bounded_and_cursor_scoped_with_cycles_and_many_parents(
    ...
    url = f"/kg/{kg}/explore?focus_id=root"
    first = s["api"]("GET", url)
    ...
    assert (
        s["api"]("GET", f"/kg/{kg}/explore?limit=31", code=422)["error"]["code"]
        == "VALIDATION_ERROR"
    )
    assert (
        s["api"]("GET", f"/kg/{kg}/explore?focus_id=missing", code=404)["error"]["code"]
        == "NOT_FOUND"
    )
    found = s["api"]("GET", f"/kg/{kg}/explore?q=찾는 별칭")
...
def test_graph_coverage_tracks_latest_reviews_and_selected_document_version(setup):
    ...
    url = f"/kg/{kg}/explore?document_version_id={version}"
...
def test_graph_coverage_rechecks_permission_without_reading_cells(setup, monkeypatch):
    ...
    kg, version = s["kg"]["kg_revision_id"], s["doc"]["version_id"]
    s["api"]("GET", f"/kg/{kg}/explore?document_version_id={version}")
