...
from kg.v3 import engine
from kg.v3.db import Problem
from kg.v3.profile import compile_profile, validate_profile
from kg.v3.readers import XlsxReader, make_reader
...
def test_make_reader_factory_fallback(reader_root, monkeypatch):
    monkeypatch.delenv("KG_V3_READER_FACTORY", raising=False)
...
@pytest.mark.skipif(importlib.util.find_spec("kg.v3.render") is None or importlib.util.find_spec("kg.v3.render.renderer") is None, reason="render module not present yet")
