...
from schema.build import build, candidates, manifest, output_path, preview
from schema.db import Problem
from tests.ops_fixture import World, variant_profile
...
def test_exports_manifest_and_build_key_reuse(world, published):
    ...
    assert len(key) == 16 and csv_result["download_url"] == f"/api/builds/{key}/download?format=csv" and not csv_result["reused"]
