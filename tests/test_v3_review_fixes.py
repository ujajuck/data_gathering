...
from kg.v3 import operations
from kg.v3 import readers as readers_module
from kg.v3.api import create_app
from kg.v3.db import Problem, encode_cursor
from kg.v3.profile import compile_regex
from kg.v3.render.client import RenderClient
from kg.v3.render.server import RenderWorker, create_render_app
from kg.v3.service import _integrity, normalize_source_ref
from tests.v3_ops_fixture import SCHEMA, World, profile_definition
...
def test_validation_errors_are_korean(tmp_path):
    ...
    with TestClient(app) as client:
        body = client.get("/api/v3/documents?limit=500").json()["error"]
        assert body["code"] == "VALIDATION_ERROR" and "200 이하" in body["message"] and body["fields"][0]["detail"].startswith("Input should")
        body = client.get("/api/v3/documents?status=bogus").json()["error"]
        assert "중 하나여야" in body["message"] and "'" not in body["message"]
        body = client.post("/api/v3/snapshots/x/applications", json={}).json()["error"]
        assert body["message"] == "profile_id은(는) 필수입니다."
        body = client.post("/api/v3/profiles/x/reparse", json={"mode": "bad"}).json()["error"]
        assert "mode은(는)" in body["message"] and "fill" in body["message"]
        assert client.get("/api/v3/settings").json()["workspace"] == "ws"
...
def test_describe_opens_workbook_once(world, monkeypatch):
    ...
    import kg.v2.readers as v2_readers

    monkeypatch.setattr(v2_readers, "file_hash", counted_hash)
