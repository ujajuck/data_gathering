// E2E 공용 도우미(계약 §8). 모든 스펙이 같은 방식으로 오류를 모으고, 작업 공간·API·파이썬 검사를 쓴다.
...
export const MAIN_PORT = process.env.SCHEMA_E2E_PORT || "8031";
export const CONTROL_PORT = process.env.SCHEMA_E2E_CONTROL_PORT || String(Number(MAIN_PORT) + 10000);
export const PYTHON = process.env.SCHEMA_E2E_PYTHON || "python";
...
export function workspaceRoot(): string {
  const marker = join(E2E_ROOT, MAIN_PORT === "8031" ? ".workspace" : `.workspace-${MAIN_PORT}`);
...
export function mutateFirstDocument(): string {
  return runPython(
    "import sys; from pathlib import Path; from examples.demo.demo import mutate_first_document; print(mutate_first_document(Path(sys.argv[1])))",
    workspaceRoot(),
  ).trim();
}
...
export function mutateRawDocument(sourceRef: string): string {
  return runPython(
    "import sys; from pathlib import Path; from examples.demo.demo import mutate_document; print(mutate_document(Path(sys.argv[1]), sys.argv[2]))",
    workspaceRoot(),
    sourceRef,
  ).trim();
}
...
// page.request로 API를 호출한다(브라우저 컨텍스트와 같은 baseURL).
