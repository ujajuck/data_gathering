...
const LOT_COUNT = 12; // 시드 문서마다 LOT 표 12행(examples/demo/demo.py)
// 스키마 ordinal 순, group 필드 제외(schema/build.py _fields).
...
async function addDocumentsToBuild(page: Page, names: string[]) {
  ...
  const toast = page.locator(".app-toast").filter({ hasText: `${names.length}개 문서 · 데이터 빌드로 이동 ›` });
...
async function generate(page: Page, buttonName: string): Promise<BuildResponse> {
  const [response] = await Promise.all([
    page.waitForResponse((r) => r.url().includes("/api/builds?") || r.url().endsWith("/api/builds")),
...
test("문서 인계 · 제외 사유 · 스키마 · 출력 Header 편집 · 미리보기/원본 보기 · CSV/XLSX/SQLite · manifest · 새 빌드", async ({ page }) => {
  ...
  for (const name of USABLE) {
    ...
    await expect(row.getByRole("cell").nth(1).locator(".app-chip")).toHaveText("사용 가능");
    await expect(row.getByRole("cell").nth(1).locator(".app-chip")).toHaveClass(/\bok\b/);
  ...
  await expect(otherRow.getByRole("cell").nth(1).locator(".app-chip")).toHaveText("제외");
  await expect(otherRow.getByRole("cell").nth(1).locator(".app-chip")).toHaveClass(/\bmuted\b/);
  ...
  await expect(shiftedRow.getByRole("cell").nth(1).locator(".app-chip")).toHaveText("제외");
  await expect(shiftedRow.getByRole("cell").nth(2)).toHaveText("검수 필요");
  await expect(shiftedRow.getByRole("cell").nth(3)).toHaveText("-");
  const draft5 = await page.evaluate(() => JSON.parse(sessionStorage.getItem("schema.build.draft") || "null"));
  ...
  const fieldChips = page.locator('[aria-label="필드 목록"] .app-chip');
  ...
  await expect(page.locator("#app-build-header-error-pressure")).toHaveText("중복된 출력 Header입니다.");
  await expect(page.locator("#app-build-header-error-temperature")).toHaveText("중복된 출력 Header입니다.");
  ...
  await expect(page.locator(".app-inline-error")).toHaveCount(0);
  await expect(nextPreview).toBeEnabled();

  // 빈 값 오류 → 되돌리면 해제.
  await headerInput(page, "시간").fill("   ");
  await expect(page.locator("#app-build-header-error-duration")).toHaveText("출력 Header를 입력하세요.");
  ...
  // 출력 설정은 세션 저장소(schema.build.columns)에 남는다.
  const saved = await page.evaluate(() => JSON.parse(sessionStorage.getItem("schema.build.columns") || "null"));
  ...
  for (let i = 0; i < EXPECTED_HEADERS.length; i++) {
    ...
    await expect(firstCells.nth(i + 1).locator(".app-build-cell > span").first()).toHaveText(cell!.text);
  ...
  await expect(context.locator(".app-chip").first()).toHaveText("발행됨");
  ...
  const reviewCell = (ref: string) => review.locator(`.app-cell[data-ref="${ref}"]`);
  await expect(reviewCell("C9")).toHaveText("150", { timeout: 60_000 });
  await expect(reviewCell("C8")).toHaveText("온도");
  await expect(review.locator(".app-overlay")).toHaveCount(2);
  ...
  const valueBox = await review.locator('.app-overlay.value[data-range="C9:C68"]').boundingBox();
  ...
  expect(csvBuild.download_url).toBe(`/api/builds/${csvBuild.build_key}/download?format=csv`);
  expect(csvBuild.manifest.row_count).toBe(rowCount);
  await expect(page.locator(".app-toast").filter({ hasText: "데이터 빌드 완료 · CSV" })).toBeVisible();
  const result = page.getByRole("region", { name: "빌드 결과" });
  await expect(result.getByRole("heading", { level: 3 })).toContainText("data.csv");
  await expect(result.getByRole("heading", { level: 3 }).locator(".app-chip")).toHaveText("완료");
  ...
  await expect(page.locator(".app-toast").filter({ hasText: "데이터 빌드 완료 · XLSX" })).toBeVisible();
  ...
  const kv = summary.locator("dl.app-kv");
  ...
  await expect(summary.locator('[aria-label="출력 컬럼"] .app-chip')).toHaveText(EXPECTED_HEADERS.map((h) => (h === "온도_C" ? "온도_C (°C)" : h === "압력_bar" ? "압력_bar (bar)" : h === "시간" ? "시간 (min)" : h)));
  ...
  expect(await page.evaluate(() => sessionStorage.getItem("schema.build.draft"))).toBeNull();
  expect(await page.evaluate(() => sessionStorage.getItem("schema.build.columns"))).toBeNull();
  errors.assertClean();
});
