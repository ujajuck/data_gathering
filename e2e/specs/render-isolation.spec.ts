...
function collectRenderResponses(page: Page): number[] {
  ...
  page.on("response", (response) => {
    if (response.request().method() === "GET" && /\/api\/snapshots\/[^/]+\/sheets\/[^/]+\/render\?/.test(response.url())) statuses.push(response.status());
  });
  return statuses;
}

const cellOf = (scope: ReturnType<Page["locator"]>, ref: string) => scope.locator(`.app-viewer .app-cell[data-ref="${ref}"]`);
...
test("첫 렌더(별도 렌더 서버) · 캐시 재접근 < 1초 · ETag/304 · Cache-Control · 이미지 asset", async ({ page }) => {
  ...
  await expect(detail.locator(".app-viewer").getByText("렌더링 중")).toHaveCount(0);
  await expect(detail.locator(".app-viewer").getByRole("alert")).toHaveCount(0);
  ...
  await expect(reopenedInMemory.locator(".app-viewer").getByText("렌더링 중")).toHaveCount(0);
  ...
  await expect(reopened.locator(".app-viewer").getByText("렌더링 중")).toHaveCount(0);
  ...
  const sheetList2 = reopened.locator(".app-side-list");

  // 창 URL 두 번: 200 + ETag → If-None-Match → 304. Cache-Control은 항상 private, no-cache(max-age·immutable 금지).
  const windowUrl = `/api/snapshots/${snapshotId}/sheets/${sheetId}/render?range=A1:Z60`;
  ...
  const tooLarge = await page.request.get(`/api/snapshots/${snapshotId}/sheets/${sheetId}/render?range=A1:ZZ500`);
  expect(tooLarge.status()).toBe(422);
  expect((await tooLarge.json()).error.code).toBe("RANGE_TOO_LARGE");

  // (4) 이미지 시트: <img loading="lazy" src="/api/snapshots/<sid>/render-assets/<sha256>.png">.
  renderStatuses.length = 0;
  ...
  expect(renderStatuses[0]).toBe(202);
  const image = reopened.locator("img.app-sheet-image");
  await expect(image).toHaveCount(1);
  await expect(image).toHaveAttribute("loading", "lazy");
  await expect(image).toHaveAttribute("alt", "시트 이미지");
  const src = (await image.getAttribute("src")) || "";
  const assetMatch = src.match(new RegExp(`^/api/snapshots/${snapshotId}/render-assets/([0-9a-f]{64}\\.png)$`));
  ...
  const imageWindow = await page.request.get(`/api/snapshots/${snapshotId}/sheets/${imageSheet.sheet_id}/render?range=A1:Z60`);
  ...
  for (const bad of ["abc.png", `${"0".repeat(64)}.svg`, `${"0".repeat(64)}.png`, assetId.toUpperCase(), assetId.replace(".png", ".png.tmp")]) {
    const missing = await page.request.get(`/api/snapshots/${snapshotId}/render-assets/${encodeURIComponent(bad)}`);
    expect(missing.status(), `asset ${bad}`).toBe(404);
  }
  const wrongSnapshot = await page.request.get(`/api/snapshots/${"0".repeat(8)}-0000-0000-0000-${"0".repeat(12)}/render-assets/${assetId}`);
...
test("격리: 큰 시트를 렌더하는 동안 문서 목록 API < 500ms · 화면 이동 가능", async ({ page }) => {
  ...
  const heavyName = runPython(
    "import sys; from pathlib import Path; from examples.demo.demo import write_heavy_document; print(write_heavy_document(Path(sys.argv[1]), rows=int(sys.argv[2]), cols=int(sys.argv[3])))",
    workspaceRoot(),
    String(HEAVY_ROWS),
    String(HEAVY_COLS),
  ).trim();
  ...
  const windowUrl = `/api/snapshots/${snapshotId}/sheets/${sheetId}/render?range=A1:Z60`;
  const documentsUrl = "/api/documents?limit=20";
  const expectedDocuments = (await api(page, "GET", documentsUrl.slice("/api".length))).json.items.length;
  ...
  const tail = await timedGet(page, `/api/snapshots/${snapshotId}/sheets/${sheetId}/render?range=A${HEAVY_ROWS - 59}:Z${HEAVY_ROWS}`);
...
test("문서 목록 API 지연 기준선(10회)", async ({ page }) => {
  ...
  const samples: number[] = [];
  for (let n = 0; n < 10; n++) {
    const sample = await timedGet(page, "/api/documents");
    expect(sample.response.status()).toBe(200);
