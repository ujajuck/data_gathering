...
test("쉘 · 문서 표 · 상태 칩 · 필터 · 정렬", async ({ page }) => {
  ...
  await expect(page.locator(".v3-brand strong")).toHaveText("Semantic Excel Integration");
  await expect(page.locator(".v3-brand small")).toHaveText("Document-to-Table Adapter");
...
test("문서 등록 대화상자 · 상세 드로어(파일 보기·추출 결과·적용 프로파일·연결 스키마) · 잠긴 문서", async ({ page }) => {
  ...
  const head = detail.locator(".v3-modal-head");
  await expect(head.locator(".v3-chip").first()).toHaveText("정상");
  ...
  const viewer = detail.locator(".v3-viewer");
  const cell = (ref: string) => viewer.locator(`.v3-cell[data-ref="${ref}"]`);
  ...
  await expect(viewer.locator(".v3-viewer-toolbar")).toContainText("공정 기록 · 60행 × 26열");
  ...
  await expect(viewer.locator(".v3-sheet")).toHaveCSS("transform", /matrix\(0\.9, 0, 0, 0\.9, 0, 0\)/);
  ...
  const cached = await page.request.get(`/api/v3/snapshots/${snapshotId}/sheets/${mainSheet.sheet_id}/render?range=A1:Z60`);
  ...
  const notModified = await page.request.get(`/api/v3/snapshots/${snapshotId}/sheets/${mainSheet.sheet_id}/render?range=A1:Z60`, {
  ...
  await expect(review.locator(`.v3-cell[data-ref="D9"]`)).toHaveText("1.2", { timeout: 60_000 });
  ...
  const schemaCard = detail.locator(".v3-list-item").filter({ hasText: SCHEMA_NAME });
  ...
  await expect(locked.locator(".v3-modal-head")).toContainText("Snapshot 없음");
...
test("새 snapshot(변경 감지) · Snapshot 이력 · 다중 선택 → 데이터 빌드", async ({ page }) => {
  ...
  const detail = await openDocument(page, REFERENCE);
  await expect(detail.locator(".v3-modal-head .v3-chip").first()).toHaveText("변경 감지");
  await expect(detail.locator(".v3-modal-head")).toContainText("최신");
  await expect(detail.locator('[aria-label="문서 관계"]')).toContainText("최근 처리");
  const history = detail.locator("details.v3-details");
  ...
  await expect(entries.nth(0).locator(".v3-chip")).toHaveText("최신");
  await expect(entries.nth(1)).toContainText(/^r1 · \d{4}-\d{2}-\d{2}/);
  await expect(entries.nth(1).locator(".v3-chip")).toHaveCount(0);
  ...
  const cell = (ref: string) => detail.locator(`.v3-viewer .v3-cell[data-ref="${ref}"]`);
  ...
  const toast = page.locator(".v3-toast").filter({ hasText: "2개 문서 · 데이터 빌드로 이동 ›" });
