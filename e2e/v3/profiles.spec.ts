...
// kg/parsing.py `_validate_spec` 모양의 v1 파싱 템플릿(sheet_templates → mappings.source.key_search).
const V1_TEMPLATE = {
  sheet_templates: [
    {
      name: "공정 기록",
...
test("목록 · 상태 필터 · 상세 탭(기본 정보 · 규칙 · 필드 매핑 · JSON · 변경 이력)", async ({ page }) => {
  ...
  await expect(row.getByRole("cell").nth(4).locator(".v3-chip")).toHaveText("승인");
  await expect(row.getByRole("cell").nth(4).locator(".v3-chip")).toHaveClass(/\bok\b/);
  ...
  await expect(detail.locator(".v3-card-head .v3-chip").first()).toHaveText("승인");
  await expect(detail.locator(".v3-card-head")).toContainText(SCHEMA_NAME);
  ...
  const sideList = page.locator(".v3-side-list[aria-label='파싱 프로파일 목록']");
  ...
  const kv = detail.locator("dl.v3-kv").first();
  const valueOf = (term: string) => kv.locator("dt", { hasText: term }).locator("xpath=following-sibling::dd[1]");
  await expect(valueOf("상태").locator(".v3-chip")).toHaveText("승인");
  ...
  await expect(valueOf("자동 승인").locator(".v3-chip")).toHaveText("활성");
  ...
  await expect(cards.locator(".v3-card-head strong")).toHaveText(RULE_NAMES);
  await expect(cards.locator(".v3-card-head code")).toHaveText(RULE_KEYS);
  await expect(cards.locator(".v3-card-head .v3-chip")).toHaveText(RULE_NAMES.map((name) => `→ ${name}`));
  ...
  await expect(detail.getByRole("status")).toHaveText("현재 리비전 v1과 같습니다.");
  await expect(detail.getByRole("button", { name: "검증" })).toBeEnabled();
  const previewRequest = page.waitForRequest((r) => r.method() === "POST" && r.url().includes("/api/v3/profiles/import-preview"));
  ...
  await expect(verdict.locator(".v3-chip")).toHaveText(["parsing-profile-3.0", "오류 없음"]);
  ...
  expect(download.url()).toMatch(new RegExp(`/api/v3/profiles/${seeded.profile_id}/export$`));
  ...
  await expect(historyRow.getByRole("cell").nth(0).locator(".v3-chip")).toHaveText("현재");
...
test("외부 Profile Import: v1 파싱 템플릿 → 형식 판별 · canonical 미리보기 · 경고 → 저장 → 초안 프로파일", async ({ page }) => {
  ...
  await expect(dialog).toContainText("v1/v2 정의나 key-value JSON을 붙여넣으면");
  ...
  await expect(preview.locator("p.v3-small .v3-chip")).toHaveText(["v1-parsing-template", "오류 없음", "경고 2"]);
  ...
  const saveRequest = page.waitForRequest((r) => r.method() === "POST" && /\/api\/v3\/profiles$/.test(r.url()));
  ...
  await expect(detail.locator(".v3-card-head .v3-chip").first()).toHaveText("초안");
  await expect(detail.locator(".v3-card-head .v3-chip").first()).toHaveClass(/\bwarn\b/);
  ...
  await expect(valueOf("자동 승인").locator(".v3-chip")).toHaveText("비활성");
  ...
  await expect(card.locator(".v3-card-head strong")).toHaveText("temperature");
  await expect(card.locator(".v3-card-head .v3-chip")).toHaveText("→ 온도");
  ...
  await expect(sideItem.locator(".v3-chip")).toHaveText("초안");
  ...
  await expect(imported.getByRole("cell").nth(4).locator(".v3-chip")).toHaveText("초안");
  await expect(imported.getByRole("cell").nth(4).locator(".v3-chip")).toHaveClass(/\bwarn\b/);
  await expect(rowOf(page, PROFILE_NAME).getByRole("cell").nth(4).locator(".v3-chip")).toHaveText("승인");
...
test("테스트 탭 → Source Review(테스트 모드) · 재파싱(fill) 작업 토스트", async ({ page }) => {
  ...
  const testRequest = page.waitForRequest((r) => r.method() === "POST" && r.url().includes(`/api/v3/profiles/${seeded.profile_id}/test`));
  await run.click();
  ...
  await expect(context.locator(".v3-chip").first()).toHaveText("테스트");
  ...
  await expect(rules.getByRole("button").locator(".v3-chip")).toHaveText(["값 1", "값 1", "값 1", "값 1", "값 12", "값 12", "값 12", "값 12", "값 12", "값 12", "값 12"]);
  ...
  await expect(panel.locator(".v3-chip").first()).toHaveText("동일");
  await expect(panel).toContainText("규칙 11개");
  await expect(panel.getByRole("alert")).toHaveCount(0);
  const panelValue = (term: string) => panel.locator("dl.v3-kv dt", { hasText: term }).locator("xpath=following-sibling::dd[1]");
  ...
  const cell = (ref: string) => review.locator(`.v3-cell[data-ref="${ref}"]`);
  await expect(cell("A3")).toHaveText("제품명", { timeout: 60_000 });
  await expect(cell("B3")).toHaveText("제품-02");
  await expect(review.locator(".v3-overlay[data-kind='key']")).toHaveCount(1);
  await expect(review.locator(".v3-overlay[data-kind='value']")).toHaveCount(1);
  await expect(review.locator(".v3-overlay[data-kind='key'] .v3-overlay-label")).toHaveText("키");
  await expect(review.locator(".v3-overlay[data-kind='value'] .v3-overlay-label")).toHaveText("값");
  ...
  await expect(review.locator(".v3-overlay[data-kind='key']")).toHaveCount(1);
  await expect(review.locator(".v3-overlay[data-kind='value']")).toHaveCount(1);
  await expect(review.locator(".v3-overlay[data-kind='unit']")).toHaveCount(0);
  ...
  await expect(review.locator(".v3-overlay[data-kind='unit']")).toHaveCount(1);
  await expect(review.locator(".v3-overlay[data-kind='value']")).toHaveCount(0);
  ...
  const rerun = page.waitForRequest((r) => r.method() === "POST" && r.url().includes(`/api/v3/profiles/${seeded.profile_id}/test`));
  ...
  const reparseRequest = page.waitForRequest((r) => r.method() === "POST" && r.url().includes(`/api/v3/profiles/${seeded.profile_id}/reparse`));
