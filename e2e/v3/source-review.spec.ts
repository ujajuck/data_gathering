...
const viewerOf = (scope: Locator) => scope.locator(".v3-viewer");
const cellOf = (scope: Locator, ref: string) => scope.locator(`.v3-cell[data-ref="${ref}"]`);
const rulesOf = (scope: Locator) => scope.getByRole("group", { name: "파싱 규칙 목록" });
const ruleItem = (page: Page, scope: Locator, name: string) => rulesOf(scope).locator(".v3-list-item").filter({ has: page.locator(`strong:text-is("${name}")`) });
const panelOf = (scope: Locator) => scope.locator('[data-testid="mapping-panel"]');
// 패널 메시지(승인됨·복원됨·반려됨…): 패널 루트 바로 아래의 v3-note(변경 이력의 '불러오는 중…' role=status와 구분).
const messageOf = (panel: Locator) => panel.locator(":scope > .v3-note[role='status']");
const kvOf = (panel: Locator, index: number) => panel.locator("dl.v3-kv dd").nth(index);
...
// 렌더 창 GET의 range=와 /api/v3 POST 경로를 모은다(창 요청 수·승인 요청 1회 검사용).
function trackRequests(page: Page) {
  ...
  page.on("request", (request) => {
    ...
    if (request.method() === "POST" && url.includes("/api/v3/")) posts.push(url.replace(/^.*\/api\/v3/, ""));
  });
...
test("검수 화면: 컨텍스트 · 시트/규칙 목록 · 실제 셀 · overlay · 확대 · 셀 이동 → 드래그 재지정 · 승인(요청 1회) · 모두 승인 · 이력/복원 · 반려 · 역방향 조회", async ({ page }) => {
  ...
  await expect(context.locator(".v3-chip")).toHaveText(["미발행", "승인 0/11", "호환"]);
  await expect(context.locator(".v3-chip").nth(0)).toHaveClass(/\bwarn\b/);
  ...
  await expect(side.locator("p.v3-small strong")).toHaveText(PROFILE_V1);
  await expect(side.locator("p.v3-small .v3-muted")).toHaveText(SCHEMA_V1);
  const rules = rulesOf(review);
  await expect(rules.locator(".v3-list-item")).toHaveCount(11);
  await expect(rules.locator(".v3-list-item strong")).toHaveText(["제품명", "레시피명", "공정명", "설비명", "배치", "측정일시", "온도", "압력", "시간", "결과값", "판정"]);
  await expect(rules.locator(".v3-list-item .v3-chip")).toHaveText(Array(11).fill("제안"));
  await expect(rules.locator(".v3-list-item .v3-chip").first()).toHaveClass(/\bwarn\b/);
  ...
  await expect(viewer.locator(".v3-viewer-toolbar")).toContainText(`${MAIN_SHEET} · 60행 × 26열`);
  await expect(viewer.getByText("렌더링 중")).toHaveCount(0);
  await expect(viewer.getByRole("alert")).toHaveCount(0);
  await expect(viewer.locator(".v3-legend")).toContainText("키");
  const cellCount = await viewer.locator(".v3-cell").count();
  ...
  await expect(overlay("unit").locator(".v3-overlay-label")).toHaveText("단위");
  ...
  await expect(viewer.locator(".v3-sheet")).toHaveCSS("transform", /matrix\(0\.75, 0, 0, 0\.75, 0, 0\)/);
  await zoom.selectOption("1");
  await expect(viewer.locator(".v3-sheet")).toHaveCSS("transform", /matrix\(1, 0, 0, 1, 0, 0\)|none/);
  await viewer.getByLabel("셀 이동").fill("G23");
  await viewer.getByRole("button", { name: "이동" }).click();
  await expect(viewer.locator(".v3-cell.focused")).toHaveAttribute("data-ref", "G23");
  ...
  await expect(panel.locator("h3 + .v3-chip")).toHaveText("제안");
  await expect(panel.locator("h3 + .v3-chip")).toHaveClass(/\bwarn\b/);
  await expect(panel.locator("dl.v3-kv dt")).toHaveText(["필드", "파싱 규칙", "관찰된 키", "추출값", "원본 위치", "상태"]);
  ...
  await expect(context.locator(".v3-chip").nth(1)).toHaveText("승인 1/11");
  ...
  await expect(panel.locator("h3 + .v3-chip")).toHaveText("승인");
  await expect(panel.locator("h3 + .v3-chip")).toHaveClass(/\bok\b/);
  ...
  await expect(ruleItem(page, review, "온도").locator(".v3-chip")).toHaveText("승인");
  await expect(overlay("value")).toHaveAttribute("data-range", "C12:C23");
  ...
  await expect(banner).toHaveClass(/\bv3-note\b/);
  expect(requests.posts).toEqual([`/applications/${applicationId}/approve-all?wait=20`]);
  await expect(context.locator(".v3-chip")).toHaveText(["발행됨", "승인 11/11", "호환"]);
  await expect(context.locator(".v3-chip").nth(0)).toHaveClass(/\bok\b/);
  await expect(context.getByRole("button", { name: "모두 승인" })).toBeDisabled();
  await expect(rules.locator(".v3-list-item .v3-chip")).toHaveText(Array(11).fill("승인"));
  ...
  await expect(context.locator(".v3-chip")).toHaveText(["미발행", "승인 10/11", "호환"]);
  ...
  await expect(panel.locator("h3 + .v3-chip")).toHaveText("반려");
  await expect(panel.locator("h3 + .v3-chip")).toHaveClass(/\berr\b/);
  await expect(kvOf(panel, 5)).toHaveText("반려 · 수동 · #4");
  await expect(panel.getByRole("button", { name: "반려", exact: true })).toBeDisabled();
  await expect(ruleItem(page, review, "온도").locator(".v3-chip")).toHaveText("반려");
...
test("창 요청: 60행·26열을 넘는 문서를 스크롤하면 뷰어가 두 번째 창(A61:… / AA…)을 요청해 그린다", async ({ page }) => {
  const errors = collectErrors(page);
  runPython("import sys; from pathlib import Path; from examples.schema_v3.demo import write_wide_document; print(write_wide_document(Path(sys.argv[1])))", workspaceRoot());
  ...
  await expect(viewer.locator(".v3-viewer-toolbar")).toContainText(`${MAIN_SHEET} · 80행 × 30열`);
  ...
  await expect(viewer.locator(".v3-overlay.value")).toHaveAttribute("data-range", "C9:C68");
...
test("잠긴 문서: 드로어 파일 보기는 Snapshot 없음 안내, 등록 뒤 잠긴 원본은 뷰어 안에서만 DRM 실패 + 다시 시도 — 나머지 화면은 정상", async ({ page }) => {
  ...
  await expect(locked.locator(".v3-modal-head .v3-chip").first()).toHaveText("잠김(DRM)");
  ...
  await expect(locked.locator(".v3-viewer")).toHaveCount(0);
  ...
  runPython(
    "import sys; from pathlib import Path; from examples.schema_v3.demo import build_locked_file; build_locked_file(Path(sys.argv[1]) / 'data/raw' / sys.argv[2])",
    workspaceRoot(),
    SWAPPED,
  );
  ...
  const denied = await page.request.get(`/api/v3/snapshots/${snapshotId}/sheets/${mainSheet.sheet_id}/render?range=A1:Z60`);
  ...
  const failure = viewer.locator(".v3-viewer-status.failed");
  ...
  await expect(viewer.locator(".v3-cell")).toHaveCount(0);
  await expect(viewer.getByText("렌더링 중")).toHaveCount(0);
  // 다른 시트로 바꿔도 같은 실패 상태(뷰어 안에서만), 드로어의 나머지 탭은 정상.
  await sheetList.getByRole("button", { name: new RegExp(COMMON_SHEET) }).click();
  await expect(viewer.locator(".v3-viewer-status.failed")).toContainText(DRM_MESSAGE, { timeout: 60_000 });
  ...
  const review = reviewDialog(page);
  await expect(review).toBeVisible();
  await expect(viewerOf(review).locator(".v3-viewer-status.failed")).toContainText(DRM_MESSAGE, { timeout: 60_000 });
  await expect(rulesOf(review).locator(".v3-list-item")).toHaveCount(11);
  await expect(rulesOf(review).locator(".v3-list-item .v3-chip")).toHaveText(Array(11).fill("승인"));
