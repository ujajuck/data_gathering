...
test("스키마 목록 · 상세 헤더 · 트리/그래프 토글 · 필드 상세 · 사용 프로파일/연관 문서 추적 · 필드에서 Source Review", async ({ page }) => {
  ...
  await expect(heading.locator(".v3-chip")).toHaveText("활성");
  await expect(heading.locator(".v3-chip")).toHaveClass(/\bok\b/);
  await expect(detail.locator(".v3-card-head .v3-muted")).toHaveText("필드 14 · 프로파일 1 · 문서 4");
  ...
  await expect(detail.locator(".v3-tree-root strong")).toHaveText(SCHEMA_NAME);
  await expect(detail.locator(".v3-tree-root")).toContainText("필드 14");
  ...
  await expect(groupItems.locator(":scope > .v3-tree-row .v3-tree-name")).toHaveText(GROUPS);
  for (const group of GROUPS) await expect(treeItem(page, group)).toHaveAttribute("aria-expanded", "true");
  await expect(tree(page).locator('[role="treeitem"][aria-level="2"] .v3-tree-name')).toHaveText(FIELDS);
  await expect(treeItem(page, "온도").locator(":scope > .v3-tree-row small")).toHaveText("decimal · °C");
  await expect(treeItem(page, "온도")).toHaveAttribute("aria-level", "2");
  await expect(treeItem(page, "공정 정보").locator('[role="group"] [role="treeitem"] .v3-tree-name')).toHaveText(["공정명", "설비명", "배치", "측정일시", "온도", "압력", "시간"]);
  ...
  await expect(svg.locator(".v3-graph-hull")).toHaveCount(3);
  for (const group of GROUPS) await expect(svg.locator(".v3-graph-hull-label", { hasText: group })).toHaveCount(1);
  await expect(svg.locator(".v3-graph-node.root")).toContainText(SCHEMA_NAME);
  const fieldNodes = svg.locator('.v3-graph-node[data-level="2"]');
  await expect(fieldNodes).toHaveCount(11);
  await expect(svg.locator('.v3-graph-node.group[data-level="1"]')).toHaveCount(3);
  ...
  await expect(kv(page, "필드명").locator(".v3-chip")).toHaveText("활성");
  ...
  await expect(field.getByLabel("Alias").locator(".v3-chip")).toHaveText(["온도값", "Temp"]);
  ...
  await expect(fieldDetail(page).getByLabel("Alias").locator(".v3-chip")).toHaveText(["Pressure"]);
  ...
  await expect(filterBar.locator(".v3-chip")).toHaveText("온도");
  ...
  await expect(profileRow.getByRole("cell").nth(4).locator(".v3-chip")).toHaveText("승인");
  ...
  await expect(detail.getByRole("status", { name: "필드 필터" }).locator(".v3-chip")).toHaveText("온도");
  ...
  for (const name of PUBLISHED_DOCUMENTS) {
    ...
    await expect(row.getByRole("cell").nth(3).locator(".v3-chip")).toHaveText("정상");
  ...
  await expect(shiftedRow.getByRole("cell").nth(3).locator(".v3-chip")).toHaveText("검수 필요");
  ...
  await expect(context.locator(".v3-chip")).toHaveText(["발행됨", "승인 11/11", "동일"]);
  ...
  await expect(ruleButton.locator(".v3-chip")).toHaveText("승인");
  ...
  const panelKv = panel.locator("dl.v3-kv").first();
  ...
  const overlayRects = review.locator(".v3-overlay");
  await expect(overlayRects).toHaveCount(2);
  await expect(review.locator('.v3-overlay.key[data-range="C8"]')).toHaveCount(1);
  await expect(review.locator('.v3-overlay.key[data-range="C8"] .v3-overlay-label')).toHaveText("키");
  await expect(review.locator('.v3-overlay.value[data-range="C9:C68"]')).toHaveCount(1);
  await expect(review.locator('.v3-overlay.value[data-range="C9:C68"] .v3-overlay-label')).toHaveText("값");
  const keyBox = await review.locator('.v3-overlay.key[data-range="C8"]').boundingBox();
  ...
  await expect(review.locator('.v3-overlay.unit[data-range="B1"]')).toHaveCount(1);
  await expect(review.locator(".v3-overlay")).toHaveCount(1);
  ...
  await expect(review).toBeVisible();
  await expect(review.locator(".v3-context").first().locator("strong")).toHaveText(SHIFTED);
  await expect(review.locator(".v3-context").first().locator(".v3-chip")).toHaveText(["미발행", "승인 0/11", "호환"]);
...
test("필드 편집(alias 추가 → PATCH → 새 리비전) · 변경 이력", async ({ page }) => {
  ...
  await expect(history.locator("tbody tr").first().locator(".v3-chip")).toHaveText("현재");
  ...
  const patchRequest = page.waitForRequest((request) => request.method() === "PATCH" && request.url().includes(`/api/v3/schemas/${SCHEMA_KEY}/fields/temperature`));
  const patchResponse = page.waitForResponse((response) => response.request().method() === "PATCH" && response.url().includes(`/api/v3/schemas/${SCHEMA_KEY}/fields/temperature`));
  ...
  await expect(page.locator(".v3-toast").filter({ hasText: "온도 필드 저장됨" })).toBeVisible();
  await expect(form).toHaveCount(0);
  await expect(field.getByLabel("Alias").locator(".v3-chip")).toHaveText(["온도값", "Temp", "온도값2"]);
  await expect(kv(page, "필드명").locator("strong")).toHaveText("온도");
  await expect(kv(page, "단위")).toHaveText("°C");
  ...
  await expect(history.locator("tbody tr").nth(0).locator(".v3-chip")).toHaveText("현재");
  await expect(history.locator("tbody tr").nth(1).getByRole("cell").nth(0)).toContainText("v1");
  await expect(history.locator("tbody tr").nth(1).locator(".v3-chip")).toHaveCount(0);
  await expect(history.locator(".v3-chip")).toHaveCount(1);
  ...
  await page.reload();
  await expect(detail.getByRole("heading", { level: 2 })).toContainText(`${SCHEMA_NAME} v2`);
  await expect(fieldDetail(page).getByLabel("Alias").locator(".v3-chip")).toHaveText(["온도값", "Temp", "온도값2"]);
  const mainText = await page.locator("main").innerText();
  expect(mainText).not.toMatch(UUID);
  errors.assertClean();
});
