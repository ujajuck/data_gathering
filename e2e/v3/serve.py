"""v3 E2E 러너(감독 프로세스) — 임시 작업 공간에 가상 문서·스키마·프로파일을 만들고 두 서버를 자식 프로세스로 띄운다.

- 렌더 서버: `python -m kg.v3 render-serve` 별도 프로세스, 127.0.0.1:8032 (KG_E2E_V3_RENDER_PORT)
- 메인 API/UI: `python serve.py --main`(kg.v3.api.create_app) 별도 프로세스, 127.0.0.1:8031 (KG_E2E_V3_PORT, KG_V3_RENDER_URL로 렌더 서버 사용)
- 제어 서버: 127.0.0.1:18031 (KG_E2E_V3_CONTROL_PORT, 기본 메인 포트+10000). `POST /reset`은 새 임시 작업 공간을 시드하고
...
작업 공간 경로는 `e2e/v3/.workspace`(기본 포트가 아니면 `.workspace-<port>`)에 기록한다(리셋마다 갱신). 스펙은 이 경로로 새 snapshot 시나리오
...
CONTROL_PORT = int(os.environ.get("KG_E2E_V3_CONTROL_PORT", str(MAIN_PORT + 10000)))
...
class Runner:
    ...
    def start(self) -> Path:
        ...
        root = Path(tempfile.mkdtemp(prefix="kg-v3-e2e-"))
        seed(root)
        self.render = subprocess.Popen(
            [sys.executable, "-m", "kg.v3", "render-serve", "--ws", str(root), "--port", str(RENDER_PORT)],
            # 하위 프로세스는 cwd(e2e/)에서 `kg`를 찾지 못하므로 저장소 루트를 PYTHONPATH로 넘긴다.
            env=CHILD_ENV,
        ...
        self.main = subprocess.Popen(
            ...
            env={**CHILD_ENV, "KG_V3_RENDER_URL": f"http://127.0.0.1:{RENDER_PORT}"},
            stderr=subprocess.PIPE,
        )
        forward_stderr(self.main, "main")
        wait_for(f"http://127.0.0.1:{MAIN_PORT}/api/v3/status", process=self.main)
        self.root = root
        self.generation += 1
