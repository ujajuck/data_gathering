...
// 묶음 행(멤버 행은 tr.v3-queue-members로 뒤에 붙는다).
const groupRow = (page: Page, label: string) => groupTable(page, label).locator("tbody > tr:not(.v3-queue-members)").first();
...
const toast = (page: Page, text: string) => page.locator(".v3-toasts").getByRole("status").filter({ hasText: text }).or(page.locator(".v3-toast").filter({ hasText: text }));
...
function writeHeavyDocument(): string {
  return runPython(
    "import sys; from pathlib import Path; from examples.schema_v3.demo import write_heavy_document; print(write_heavy_document(Path(sys.argv[1]), rows=4000, cols=150))",
    workspaceRoot(),
  ).trim();
}
...
test("요약 카드 · 신규 양식/매핑 검수/파싱 실패 큐 · 렌더 서버 상태 · 작업 목록", async ({ page }) => {
  ...
  await expect(otherMember.getByRole("cell").nth(2).locator(".v3-chip")).toHaveText("프로파일 없음");
  ...
  await expect(reviewRow.getByRole("cell").nth(3).locator(".v3-chip")).toHaveText(["duration", "equipment", "lot", "measured_at", "pressure"]);
  ...
  await expect(shiftedMember.getByRole("cell").nth(2).locator(".v3-chip")).toHaveText("검수 필요");
  await expect(shiftedMember.getByRole("cell").nth(2).locator(".v3-chip")).toHaveClass(/\bwarn\b/);
  ...
  await expect(lockedMember.getByRole("cell").nth(2).locator(".v3-chip")).toHaveText("잠김(DRM)");
  await expect(lockedMember.getByRole("cell").nth(2).locator(".v3-chip")).toHaveClass(/\berr\b/);
  await expect(lockedMember.getByRole("cell").nth(2).locator(".v3-chip")).toHaveAttribute("title", DRM_MESSAGE);
  ...
  await expect(kindSelect.locator("option")).toHaveText(["전체", "등록", "추출", "재파싱", "빌드", "테스트", "묶음 처리", "마이그레이션"]);
  ...
  await expect(jobRows(page).locator("td:nth-child(3) .v3-chip")).toHaveText(["완료", "완료", "완료", "완료", "완료", "완료"]);
  await expect(jobRows(page).first().locator("td:nth-child(3) .v3-chip")).toHaveClass(/\bok\b/);
...
test("매핑 검수 전체 승인 → 문서 정상 · 큐 0 · 묶음 처리 작업 행", async ({ page }) => {
  ...
  await expect(first.getByRole("cell").nth(2).locator(".v3-chip")).toHaveText("완료");
...
test("새 snapshot(변경 감지) 큐 → 동일 승계 전체 승인 → 정상 · 변경 감지 0", async ({ page }) => {
  ...
  await expect(member.getByRole("cell").nth(2).locator(".v3-chip")).toHaveText("변경 감지");
  await expect(member.getByRole("cell").nth(2).locator(".v3-chip")).toHaveClass(/\bwarn\b/);
...
test("실패한 등록 작업 행(오류 문구 · 이동 → 문서 드로어) · 재파싱이 도는 동안 JobBar '진행 중 작업'", async ({ page }) => {
  ...
  await expect(row).toHaveClass(/\bv3-job-failed\b/);
  await expect(row.getByRole("cell").nth(0)).toHaveText("등록");
  await expect(row.getByRole("cell").nth(1)).toHaveText(LOCKED);
  await expect(row.getByRole("cell").nth(2).locator(".v3-chip")).toHaveText("실패");
  await expect(row.getByRole("cell").nth(2).locator(".v3-chip")).toHaveClass(/\berr\b/);
  await expect(row.locator(".v3-error-text")).toHaveText(DRM_MESSAGE);
  ...
  await expect(detail.locator(".v3-modal-head .v3-chip").first()).toHaveText("잠김(DRM)");
  ...
  const jobBar = page.locator(".v3-jobbar");
  ...
  await expect(running.getByRole("cell").nth(2).locator(".v3-chip")).toHaveText(/^(진행 중|대기)/);
  ...
  await expect(running.getByRole("cell").nth(2).locator(".v3-chip")).toHaveText("완료");
