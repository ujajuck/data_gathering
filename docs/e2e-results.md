...
# 브라우저 E2E (list reporter 출력을 e2e/test-results/run.log에 남긴다)
cd e2e && mkdir -p test-results
SCHEMA_E2E_CHROMIUM_PATH=<chromium 실행 파일> SCHEMA_E2E_PYTHON=python3 npm test 2>&1 | tee test-results/run.log
```
...
| 안정성 | 이름 정리가 끝난 현재 트리에서 같은 명령을 3회 실행: 21/21(2.7m) · 21/21(2.7m) · 21/21(2.7m), 재시도 0. §3·§5는 3회차(최종 트리) 출력이다 |
...
[WebServer] [e2e-control] generation 1 · workspace schema-e2e-b4zmbzpc · main 8031 · render 8032 · control 18031

Running 21 tests using 1 worker

[WebServer] [e2e-control] reset → generation 2 · workspace schema-e2e-p8x4gp_k · 3212 ms
  ✓   1 specs/build.spec.ts:91:1 › 문서 인계 · 제외 사유 · 스키마 · 출력 Header 편집 · 미리보기/원본 보기 · CSV/XLSX/SQLite · manifest · 새 빌드 (8.2s)
[WebServer] [e2e-control] reset → generation 3 · workspace schema-e2e-bthwqv15 · 3206 ms
  ✓   2 specs/documents.spec.ts:34:1 › 쉘 · 문서 표 · 상태 칩 · 필터 · 정렬 (3.2s)
  ✓   3 specs/documents.spec.ts:140:1 › 문서 등록 대화상자 · 상세 드로어(파일 보기·추출 결과·적용 프로파일·연결 스키마) · 잠긴 문서 (8.9s)
  ✓   4 specs/documents.spec.ts:363:1 › 새 snapshot(변경 감지) · Snapshot 이력 · 다중 선택 → 데이터 빌드 (5.0s)
[WebServer] [e2e-control] reset → generation 4 · workspace schema-e2e-3e0xucju · 3261 ms
  ✓   5 specs/jobs.spec.ts:72:1 › 요약 카드 · 신규 양식/매핑 검수/파싱 실패 큐 · 렌더 서버 상태 · 작업 목록 (4.7s)
  ✓   6 specs/jobs.spec.ts:265:1 › 매핑 검수 전체 승인 → 문서 정상 · 큐 0 · 묶음 처리 작업 행 (2.9s)
  ✓   7 specs/jobs.spec.ts:305:1 › 새 snapshot(변경 감지) 큐 → 동일 승계 전체 승인 → 정상 · 변경 감지 0 (4.2s)
  ✓   8 specs/jobs.spec.ts:378:1 › 실패한 등록 작업 행(오류 문구 · 이동 → 문서 드로어) · 재파싱이 도는 동안 JobBar '진행 중 작업' (21.8s)
[WebServer] [e2e-control] reset → generation 5 · workspace schema-e2e-opn0bnsh · 3403 ms
  ✓   9 specs/profiles.spec.ts:64:1 › 목록 · 상태 필터 · 상세 탭(기본 정보 · 규칙 · 필드 매핑 · JSON · 변경 이력) (9.1s)
  ✓  10 specs/profiles.spec.ts:310:1 › 외부 Profile Import: v1 파싱 템플릿 → 형식 판별 · canonical 미리보기 · 경고 → 저장 → 초안 프로파일 (3.0s)
  ✓  11 specs/profiles.spec.ts:439:1 › 테스트 탭 → Source Review(테스트 모드) · 재파싱(fill) 작업 토스트 (7.1s)
[WebServer] [e2e-control] reset → generation 6 · workspace schema-e2e-pb07kb95 · 3224 ms
  ✓  12 specs/register-directory.spec.ts:52:1 › 폴더 일괄 등록: 미리보기 → 4개 등록 → 재스캔(변경 없음) → 다시 읽기 → 값 변경(변경 감지) (5.2s)
  ✓  13 specs/register-directory.spec.ts:163:1 › API: 폴더 경로 검증(422 INVALID_SOURCE · 404) · 작업 라벨 (1.3s)
[WebServer] [e2e-control] reset → generation 7 · workspace schema-e2e-tlw6bxut · 3400 ms
[timing] first render (open drawer → cells visible, 공정 기록): 1242 ms
[timing] first render polling round-trips (202 count): 1 responses
[timing] cached re-access (close → reopen drawer, in-memory client cache): 255 ms
[timing] cached re-access (after reload → open drawer → cells visible, render-server cache): 305 ms
[timing] cached window GET (200): 16 ms
...
[timing] register heavy document (2000x90): 1982 ms
[timing] render request → 202 (uncached heavy sheet): 14 ms
[timing] documents list during render (5 samples, max): 12 ms
[timing] documents list during render (5 samples, mean): 11 ms
[timing] navigate 문서 → 파싱 스키마 during render: 437 ms
[timing] heavy sheet render (202 → 200, 2000x90): 3420 ms
[timing] heavy sheet tail window GET (cached): 79 ms
  ✓  15 specs/render-isolation.spec.ts:233:1 › 격리: 큰 시트를 렌더하는 동안 문서 목록 API < 500ms · 화면 이동 가능 (9.1s)
[timing] documents list baseline (10 samples, mean): 11 ms
[timing] documents list baseline (10 samples, max): 12 ms
[timing] documents list baseline (10 samples, min): 10 ms
[timing] summary
  first render (open drawer → cells visible, 공정 기록): 1242 ms
  first render polling round-trips (202 count): 1 responses
  cached re-access (close → reopen drawer, in-memory client cache): 255 ms
  cached re-access (after reload → open drawer → cells visible, render-server cache): 305 ms
  cached window GET (200): 16 ms
  ...
  register heavy document (2000x90): 1982 ms
  render request → 202 (uncached heavy sheet): 14 ms
  documents list during render (5 samples, max): 12 ms
  documents list during render (5 samples, mean): 11 ms
  navigate 문서 → 파싱 스키마 during render: 437 ms
  heavy sheet render (202 → 200, 2000x90): 3420 ms
  heavy sheet tail window GET (cached): 79 ms
  documents list baseline (10 samples, mean): 11 ms
  documents list baseline (10 samples, max): 12 ms
  documents list baseline (10 samples, min): 10 ms
  ✓  16 specs/render-isolation.spec.ts:331:1 › 문서 목록 API 지연 기준선(10회) (1.3s)
[WebServer] [e2e-control] reset → generation 8 · workspace schema-e2e-jp4sm0mf · 3236 ms
  ✓  17 specs/schema.spec.ts:42:1 › 스키마 목록 · 상세 헤더 · 트리/그래프 토글 · 필드 상세 · 사용 프로파일/연관 문서 추적 · 필드에서 Source Review (7.7s)
  ✓  18 specs/schema.spec.ts:347:1 › 필드 편집(alias 추가 → PATCH → 새 리비전) · 변경 이력 (3.4s)
[WebServer] [e2e-control] reset → generation 9 · workspace schema-e2e-lziroibw · 3315 ms
  ✓  19 specs/source-review.spec.ts:85:1 › 검수 화면: 컨텍스트 · 시트/규칙 목록 · 실제 셀 · overlay · 확대 · 셀 이동 → 드래그 재지정 · 승인(요청 1회) · 모두 승인 · 이력/복원 · 반려 · 역방향 조회 (11.0s)
  ✓  20 specs/source-review.spec.ts:385:1 › 창 요청: 60행·26열을 넘는 문서를 스크롤하면 뷰어가 두 번째 창(A61:… / AA…)을 요청해 그린다 (3.4s)
  ✓  21 specs/source-review.spec.ts:436:1 › 잠긴 문서: 드로어 파일 보기는 Snapshot 없음 안내, 등록 뒤 잠긴 원본은 뷰어 안에서만 DRM 실패 + 다시 시도 — 나머지 화면은 정상 (3.5s)
...
| 측정 | 값 | 기준 |
| --- | --- | --- |
| 첫 렌더(드로어 열기 → 셀 표시, 공정 기록) | 1242 ms | 202 폴링 1회 |
| 첫 렌더 202 응답 수 | 1 | — |
| 캐시 재접근(드로어 닫고 다시 열기, 클라이언트 메모리 캐시) | 255 ms | < 1초 (네트워크 요청 0) |
| 캐시 재접근(새로고침 뒤 드로어 → 셀 표시, 렌더 서버 캐시) | 305 ms | < 1초 (200만, 202 없음) |
| 캐시된 창 GET (200) | 16 ms | — |
...
| 큰 문서 등록(2000×90 셀, wait=30 작업) | 1982 ms | — |
| 캐시 안 된 큰 시트 렌더 요청 → 202 | 14 ms | 절대 렌더 완료를 기다리지 않음 |
| 렌더 중 문서 목록 API (5회 최대) | 12 ms | < 500 ms |
| 렌더 중 문서 목록 API (5회 평균) | 11 ms | — |
| 렌더 중 화면 이동 문서 → 파싱 스키마 | 437 ms | 막히지 않음 |
| 큰 시트 렌더 완료(202 → 200, 2000×90) | 3420 ms | truncated false (§5 상한 200,000셀 미만) |
| 큰 시트 마지막 창 GET(캐시) | 79 ms | — |
| 문서 목록 API 기준선(10회 평균 / 최대 / 최소) | 11 / 12 / 10 ms | — |
...
| 재파싱(rematch) 큰 호환 문서(4000×150) 포함 | JobBar `진행 중 작업`이 관찰될 만큼 지속(jobs #4 전체 21.8초) |
| 잠긴 파일 단독 재등록 실패 작업 | 인라인 완료(DRM_READER_REQUIRED) |
| `POST /builds` 36행 × 11열 CSV/XLSX/SQLite | 동기 200, 두 번째 CSV 빌드 `reused: true` |
| 폴더 일괄 등록: 스캔 → 4파일 등록 → 재스캔 → 4파일 재읽기 → 값 변경 → 1파일 등록 | 세 번의 `?wait=10` 작업이 모두 대화상자 안에서 인라인 완료(register-directory #1 전체 5.2초) |
...
4. `mkdir -p test-results && SCHEMA_E2E_CHROMIUM_PATH=<chromium> SCHEMA_E2E_PYTHON=python3 npm test 2>&1 | tee test-results/run.log`
