...
## 2026-09-15 — 문서 상세 한 건 재파싱 (claude/system-redesign-e2e-docs)

- **같은 프로파일이라도 이 문서 하나만 다시 파싱한다** (이 커밋)
  - **막다른 길**: 재파싱은 `POST /profiles/{id}/reparse` 하나뿐이라 그 프로파일을 쓰는 **모든 문서**가 함께 돌았다.
    문서가 바뀌었거나 프로파일을 올려 **한 문서만** 다시 맞추고 싶어도 길이 없었다 —
    `POST /applications/{aid}/extract`는 백엔드에만 있고 화면에서 부르는 곳이 없었고, 문서 상세의 적용 프로파일 행에는
    `원본 보기`와 프로파일 열기뿐이었다(`다른 프로파일로 파싱`은 **다른** 프로파일을 고르는 길이다)
  - **한 건 재파싱(§4.9)**: `POST /applications/{aid}/reparse`(작업 · `wait` 지원). 프로파일 전체 재파싱이 문서마다 부르는
    `_reparse_application`을 **그대로** 부른다 — 판정 규칙을 새로 쓰지 않았다(전체와 한 건이 다르게 동작하면 안 된다).
    대상 집계도 전체 경로와 같은 행(`_application_aggregates`) 하나다. mode는 `rematch` 고정(버튼이 하나다).
    프로파일이 approved가 아니면 전체 경로와 같이 422 `PROFILE_NOT_APPROVED`이되 **헤드가 전부 approved면 재추출만** 하고
    응답의 `action`이 어느 쪽을 했는지 말한다. 진행 중 작업 가드는 문서 삭제(§4.13)와 **같은 함수**(`_busy_guard`)로 합쳤다
  - **화면**: 문서 상세 드로어 적용 프로파일 탭의 행마다 `다시 파싱`. 도는 동안 누른 행은 `다시 파싱 중…`이고 표의 다른
    `다시 파싱`도 함께 잠긴다(한 번에 한 건 — 잠긴 버튼의 title이 이유를 말한다). 서버가 422로 막을 행(폐기 프로파일·
    검수가 덜 끝난 초안)은 누르기 **전에** 비활성이고 title이 이유를 말한다. 10초를 넘기면 드로어를 닫아도 상단 진행 중
    작업에서 이어진다(`useJob` 그대로). 끝나면 토스트가 결과를 사람 말로 말하고(`… 다시 파싱 완료 · 값 N개` /
    추출까지 가지 않았으면 `… 다시 파싱 완료 · 검수가 필요합니다` / `… 건너뜀 · 이미 최신입니다 — …`)
    적용 프로파일·추출 결과·문서 목록을 다시 읽는다.
    **문서 목록에는 아무것도 더하지 않았다** — 목록에서의 묶음 재파싱은 §4.11 검수 큐가 하는 일이다
  - **리뷰에서 고친 것**: ① 추출까지 가지 않은 재매치(compatible → 제안 리비전만)가 `다시 파싱 완료`로만 끝나
    값이 갱신된 줄 알게 했다 → `· 검수가 필요합니다` + `원본 보기` 행동. ② 폐기된 프로파일의 적용 건이 재추출 분기로
    들어가 발행분을 덮어썼다 → 422 `PROFILE_DEPRECATED`(폐기는 기록을 그대로 둔다). ③ `up_to_date`는 원본을 읽지 않고
    판정하므로 문구가 `원본 파일이 바뀌었으면 문서를 다시 등록하세요`까지 말하고 버튼 title에서 추출 약속을 뺐다.
    ④ `ACCESS_DENIED`·`DRM_READER_REQUIRED`·`READER_*`에 우리말 문구를 더했다(영문 코드 노출). ⑤ 막히는 행을 미리
    비활성으로(`profile.status`는 이미 응답에 있었다). ⑥ 취소된 작업을 '계속 진행됩니다'라고 말하던 거짓 문구를 고쳤다
  - **E2E**: 새 스펙 파일 `e2e/specs/reparse-application.spec.ts`(test() 2개). 정의 JSON에서 LOT 표 값 영역을
    60행 → 6행으로 좁혀 v2를 저장하고 대표 문서로 다시 승인한 뒤(승인이 큐에 넣는 **프로파일 전체** 재파싱은 취소한다 —
    그래야 '한 건만'을 보일 수 있다), 문서 상세에서 `다시 파싱` → 그 문서만 값 46개·온도 6행으로 다시 발행되는 것을
    추출 결과 탭과 API로 확인하고, 한 번 더 누르면 `이미 최신입니다`로 건너뛰며, 같은 프로파일을 쓰는 다른 문서는
    r1·60행·값 50행 그대로임을 확인한다. 기존 스펙은 하나도 고치지 않았다
  - 문서: 계약 §4.9(한 건 경로 절 · 판정이 전체와 같다는 것) · §6 API 표 · §7 문서 상세 행 · §8 스펙 · §10 테스트 기준,
    아키텍처 §2(모듈) · §2.6(같은 가드) · **§3 API 지도**(문서 상세 행에 쓰기 경로) · §5 테스트 표, `e2e/README.md` 스펙 표
  - 검증(리뷰 수정 반영 뒤 다시 돌린 최종): `python3 -m pytest tests -q -p no:cacheprovider`
    **309 passed, 2 warnings, 50 subtests passed**(55.69s) · `cd frontend && npx vitest run` **15 files / 187 tests passed** ·
    `cd frontend && npm run build` 성공(`tsc -b` 포함) · 브라우저 E2E `28 passed (3.1m)`, 결과는 [e2e-results.md](e2e-results.md)
