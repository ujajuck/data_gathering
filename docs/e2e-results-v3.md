...
각 스펙 파일은 `test.beforeAll`에서 `helpers.resetWorkspace()`를 불러 시드 상태에서 시작한다(리셋 1회 ≈ 4.0–4.5초).
...
| 소요 | Playwright 보고 2.7분 (시드·서버 기동·리셋 8회 포함) |
| 안정성 | 폴더 일괄 등록 스펙을 더한 뒤 같은 명령을 연속 2회 실행: 21/21(2.6m) · 21/21(2.7m), 재시도 0. 그 직전 실행에서는 3개가 실패했고(§7.3) 원인을 고친 뒤 두 번 모두 통과했다 |
| v2 회귀 | `npm run test:v2` 3 passed (21.4s; graph.spec 1 · workbench.spec 2, 포트 8021) |
...
[WebServer] [e2e-control] generation 1 · workspace kg-v3-e2e-_izczsb2 · main 8031 · render 8032 · control 18031

Running 21 tests using 1 worker

[WebServer] [e2e-control] reset → generation 2 · workspace kg-v3-e2e-996jtnw7 · 4333 ms
  ✓   1 v3/build.spec.ts:91:1 › 문서 인계 · 제외 사유 · 스키마 · 출력 Header 편집 · 미리보기/원본 보기 · CSV/XLSX/SQLite · manifest · 새 빌드 (8.3s)
[WebServer] [e2e-control] reset → generation 3 · workspace kg-v3-e2e-vno7agup · 4525 ms
  ✓   2 v3/documents.spec.ts:34:1 › 쉘 · 문서 표 · 상태 칩 · 필터 · 정렬 (3.3s)
  ✓   3 v3/documents.spec.ts:140:1 › 문서 등록 대화상자 · 상세 드로어(파일 보기·추출 결과·적용 프로파일·연결 스키마) · 잠긴 문서 (8.9s)
  ✓   4 v3/documents.spec.ts:363:1 › 새 snapshot(변경 감지) · Snapshot 이력 · 다중 선택 → 데이터 빌드 (4.6s)
[WebServer] [e2e-control] reset → generation 4 · workspace kg-v3-e2e-e73ec4nk · 4414 ms
  ✓   5 v3/jobs.spec.ts:72:1 › 요약 카드 · 신규 양식/매핑 검수/파싱 실패 큐 · 렌더 서버 상태 · 작업 목록 (4.7s)
  ✓   6 v3/jobs.spec.ts:265:1 › 매핑 검수 전체 승인 → 문서 정상 · 큐 0 · 묶음 처리 작업 행 (2.9s)
  ✓   7 v3/jobs.spec.ts:305:1 › 새 snapshot(변경 감지) 큐 → 동일 승계 전체 승인 → 정상 · 변경 감지 0 (4.2s)
  ✓   8 v3/jobs.spec.ts:378:1 › 실패한 등록 작업 행(오류 문구 · 이동 → 문서 드로어) · 재파싱이 도는 동안 JobBar '진행 중 작업' (20.0s)
[WebServer] [e2e-control] reset → generation 5 · workspace kg-v3-e2e-huq93ver · 4168 ms
  ✓   9 v3/profiles.spec.ts:64:1 › 목록 · 상태 필터 · 상세 탭(기본 정보 · 규칙 · 필드 매핑 · JSON · 변경 이력) (7.4s)
  ✓  10 v3/profiles.spec.ts:310:1 › 외부 Profile Import: v1 파싱 템플릿 → 형식 판별 · canonical 미리보기 · 경고 → 저장 → 초안 프로파일 (2.8s)
  ✓  11 v3/profiles.spec.ts:439:1 › 테스트 탭 → Source Review(테스트 모드) · 재파싱(fill) 작업 토스트 (6.8s)
[WebServer] [e2e-control] reset → generation 6 · workspace kg-v3-e2e-93n_3aic · 4377 ms
  ✓  12 v3/register-directory.spec.ts:61:1 › 폴더 일괄 등록: 미리보기 → 4개 등록 → 재스캔(변경 없음) → 다시 읽기 → 값 변경(변경 감지) (4.9s)
  ✓  13 v3/register-directory.spec.ts:155:1 › API: `..`·절대 경로는 422 INVALID_SOURCE, 없는 폴더는 404 (1.1s)
[WebServer] [e2e-control] reset → generation 7 · workspace kg-v3-e2e-hj9agoxe · 4381 ms
[timing] first render (open drawer → cells visible, 공정 기록): 1261 ms
...
[timing] cached re-access (after reload → open drawer → cells visible, render-server cache): 264 ms
[timing] cached window GET (200): 20 ms
[timing] cached window GET with If-None-Match (304): 14 ms
[timing] first render (image sheet 첨부 → cells visible): 974 ms
[timing] render asset GET (200 image/png): 14 ms
  ✓  14 v3/render-isolation.spec.ts:72:1 › 첫 렌더(별도 렌더 서버) · 캐시 재접근 < 1초 · ETag/304 · Cache-Control · 이미지 asset (5.6s)
[timing] register heavy document (2000x90): 1643 ms
[timing] render request → 202 (uncached heavy sheet): 12 ms
[timing] documents list during render (5 samples, max): 12 ms
[timing] documents list during render (5 samples, mean): 10 ms
[timing] navigate 문서 → 파싱 스키마 during render: 460 ms
[timing] heavy sheet render (202 → 200, 2000x90): 2770 ms
[timing] heavy sheet tail window GET (cached): 37 ms
  ✓  15 v3/render-isolation.spec.ts:233:1 › 격리: 큰 시트를 렌더하는 동안 문서 목록 API < 500ms · 화면 이동 가능 (7.7s)
[timing] documents list baseline (10 samples, mean): 9 ms
[timing] documents list baseline (10 samples, max): 16 ms
[timing] documents list baseline (10 samples, min): 7 ms
[timing] summary
  first render (open drawer → cells visible, 공정 기록): 1261 ms
  first render polling round-trips (202 count): 1 responses
  cached re-access (close → reopen drawer, in-memory client cache): 258 ms
  cached re-access (after reload → open drawer → cells visible, render-server cache): 264 ms
  cached window GET (200): 20 ms
  cached window GET with If-None-Match (304): 14 ms
  first render (image sheet 첨부 → cells visible): 974 ms
  render asset GET (200 image/png): 14 ms
  register heavy document (2000x90): 1643 ms
  render request → 202 (uncached heavy sheet): 12 ms
  documents list during render (5 samples, max): 12 ms
  documents list during render (5 samples, mean): 10 ms
  navigate 문서 → 파싱 스키마 during render: 460 ms
  heavy sheet render (202 → 200, 2000x90): 2770 ms
  heavy sheet tail window GET (cached): 37 ms
  documents list baseline (10 samples, mean): 9 ms
  documents list baseline (10 samples, max): 16 ms
  documents list baseline (10 samples, min): 7 ms
  ✓  16 v3/render-isolation.spec.ts:331:1 › 문서 목록 API 지연 기준선(10회) (1.2s)
[WebServer] [e2e-control] reset → generation 8 · workspace kg-v3-e2e-f4t0rsam · 4266 ms
  ✓  17 v3/schema.spec.ts:42:1 › 스키마 목록 · 상세 헤더 · 트리/그래프 토글 · 필드 상세 · 사용 프로파일/연관 문서 추적 · 필드에서 Source Review (7.0s)
  ✓  18 v3/schema.spec.ts:347:1 › 필드 편집(alias 추가 → PATCH → 새 리비전) · 변경 이력 (2.9s)
[WebServer] [e2e-control] reset → generation 9 · workspace kg-v3-e2e-tsn097ng · 4012 ms
  ✓  19 v3/source-review.spec.ts:85:1 › 검수 화면: 컨텍스트 · 시트/규칙 목록 · 실제 셀 · overlay · 확대 · 셀 이동 → 드래그 재지정 · 승인(요청 1회) · 모두 승인 · 이력/복원 · 반려 · 역방향 조회 (9.5s)
  ✓  20 v3/source-review.spec.ts:385:1 › 창 요청: 60행·26열을 넘는 문서를 스크롤하면 뷰어가 두 번째 창(A61:… / AA…)을 요청해 그린다 (2.9s)
  ✓  21 v3/source-review.spec.ts:436:1 › 잠긴 문서: 드로어 파일 보기는 Snapshot 없음 안내, 등록 뒤 잠긴 원본은 뷰어 안에서만 DRM 실패 + 다시 시도 — 나머지 화면은 정상 (3.5s)
...
| Document | 원본 폴더 하나를 지정하면 하위 파일 전부 등록(§4.1.1) | register-directory #1 | 등록 대화상자 → `일괄` 폴더 → `이 폴더 전체 등록` → 미리보기 `일괄/ 아래 파일 4개 · 하위 폴더 2개` · 칩 `새 파일 4` · `건너뜀: 임시 파일 1 · 지원하지 않는 파일 1` → `4개 등록 시작` → 요약 `4개 중 4개 등록 · 0개 변경 없음 · 0개 실패` · 결과 표 4행 → 문서 목록 4건(정상 3 · 프로파일 없음 1) | pass |
| Document | 재실행이 싸다(변경 없는 파일 건너뜀) | register-directory #1 | 같은 폴더 재스캔 → 칩 `변경 없음 4` · `0개 등록 시작` 비활성 · 안내 문구; `GET /sources/scan`이 `targeted: 0`; 체크박스 `변경 없는 문서·잠긴 문서도 다시 읽기` → `4개 등록 시작` → `4개 중 4개 등록 · 4개 변경 없음 · 0개 실패`(문서 수 불변) | pass |
| Document | 값이 바뀐 파일만 다시 읽는다 | register-directory #1 | 하위 폴더 복사본 1개의 값 변경 → 스캔 칩 `변경된 문서 1` · `변경 없음 3` → `1개 등록 시작` → `1개 중 1개 등록 · 3개 변경 없음 · 0개 실패` → 문서 상태 `변경 감지` | pass |
| Document | 폴더 경로 검증 | register-directory #2 | API `GET /sources/scan`·`POST /documents/register-directory`에 `..`·`일괄/../..`·`/etc` → 422 `INVALID_SOURCE`, 없는 폴더 → 404 `SOURCE_NOT_FOUND`, 알 수 없는 provider → 422 `UNKNOWN_PROVIDER`(문서 행 증가 0) | pass |
...
## 5. 측정 시간 (render-isolation 스펙, 최종 실행의 `[timing]` 값)

| 측정 | 값 | 기준 |
| --- | --- | --- |
| 첫 렌더(드로어 열기 → 셀 표시, 공정 기록) | 1261 ms | 202 폴링 1회 |
| 첫 렌더 202 응답 수 | 1 | — |
| 캐시 재접근(드로어 닫고 다시 열기, 클라이언트 메모리 캐시) | 258 ms | < 1초 (네트워크 요청 0) |
| 캐시 재접근(새로고침 뒤 드로어 → 셀 표시, 렌더 서버 캐시) | 264 ms | < 1초 (200만, 202 없음) |
| 캐시된 창 GET (200) | 20 ms | — |
| 캐시된 창 GET + If-None-Match (304) | 14 ms | ETag 일치 → 304, `Cache-Control: private, no-cache` |
| 첫 렌더(이미지 시트 첨부 → 셀 표시) | 974 ms | 이미지는 별도 asset |
| 렌더 asset GET (200 image/png) | 14 ms | — |
| 큰 문서 등록(2000×90 셀, wait=30 작업) | 1643 ms | — |
| 캐시 안 된 큰 시트 렌더 요청 → 202 | 12 ms | 절대 렌더 완료를 기다리지 않음 |
| 렌더 중 문서 목록 API (5회 최대) | 12 ms | < 500 ms |
| 렌더 중 문서 목록 API (5회 평균) | 10 ms | — |
| 렌더 중 화면 이동 문서 → 파싱 스키마 | 460 ms | 막히지 않음 |
| 큰 시트 렌더 완료(202 → 200, 2000×90) | 2770 ms | truncated false (§5 상한 200,000셀 미만) |
| 큰 시트 마지막 창 GET(캐시) | 37 ms | — |
| 문서 목록 API 기준선(10회 평균 / 최대 / 최소) | 9 / 16 / 7 ms | — |

다른 스펙에서 관찰한 값(단언 대상은 아니며 참고용):

| 측정 | 값 |
| --- | --- |
| 작업 공간 리셋(시드 + 렌더/메인 서버 재기동) | 4.0–4.5초 × 8회 |
| `POST /applications/{id}/approve-all?wait=20` (값 88개 추출 포함) | 인라인 완료, 추출 작업 ≈ 235 ms |
| `POST /profiles/{id}/reparse?wait=10` (fill, 시드 4문서 전부 건너뜀) | ≈ 20 ms |
| `POST /profiles/{id}/test` (공정데이터_2024_02, 그룹 11) | 동기 응답, 20초 TEST_TIMEOUT 안 |
| 재파싱(rematch) 시드 5문서 | ≈ 290 ms; 큰 호환 문서(4000×150) 포함 시 ≈ 3–4초 (JobBar `진행 중 작업` 관찰용) |
| 잠긴 파일 단독 재등록 실패 작업 | ≈ 170 ms (DRM_READER_REQUIRED) |
| `POST /builds` 36행 × 11열 CSV/XLSX/SQLite | 동기 200, 두 번째 CSV 빌드 `reused: true` |
| 폴더 일괄 등록 4파일(등록+자동 적용+추출, `wait=10`) | 대화상자 안에서 인라인 완료(스펙 전체 4.9초) |
| 같은 폴더 재스캔(`GET /sources/scan`, 4파일·2폴더) | Reader 없이 stat + `source_digest` 재사용 — 인라인 응답 |
...
| `register-directory.spec.ts` | 2 | 원본 폴더에 `일괄/2024/`·`일괄/2024/하위/` 트리(같은 양식 복사본 3 + 다른 양식 1 + `~$임시.xlsx` + `메모.txt`) 생성 · 대화상자에서 폴더 이동 → `이 폴더 전체 등록` 미리보기(파일/하위 폴더 수 · 상태 칩 · 건너뜀 · 다시 읽기 체크박스) → `4개 등록 시작` → 요약·결과 표·문서 목록 4건(정상 3 · 프로파일 없음 1) · 재스캔 `변경 없음 4`와 시작 비활성 · 다시 읽기 4건(문서 수 불변) · 복사본 값 변경 → `변경된 문서 1` → 등록 → `변경 감지` · API `..`/절대 경로 422 `INVALID_SOURCE` · 없는 폴더 404 · 알 수 없는 provider 422 `UNKNOWN_PROVIDER` |
...
- **JobBar `진행 중 작업`**과 **렌더 격리 창**은 시드 문서로는 너무 빨라(재파싱 ≈ 0.3초, 시드 시트 렌더 < 1초) 관찰할 수 없어 `write_heavy_document`(4000×150 / 2000×90)로 만든 큰 문서를 쓴다.
...
프런트(`frontend/src/v3/`, 재빌드·vitest 180 통과):
...
백엔드(`kg/v3/`, `pytest tests/test_v3_*.py tests/test_schema_v3.py` 185 통과):
...
수정 뒤 같은 명령을 두 번 돌려 21/21·21/21로 통과했다(§2 안정성). 폴더 일괄 등록의 500행 `documents[]`는 여전히 목록에서 빠지고
`documents_count`로 대신하며, 전문은 `GET /jobs/{id}`가 준다.
...
2. 저장소 루트에서 `python3 -m pytest tests/test_v3_*.py tests/test_schema_v3.py -q -p no:cacheprovider` → 204 passed.
