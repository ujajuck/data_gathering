...
cd frontend && npm run build && npx vitest run           # 22 files / 180 tests passed
cd .. && python3 -m pytest tests/test_v3_*.py tests/test_schema_v3.py -q -p no:cacheprovider   # 185 passed, 47 subtests
...
각 스펙 파일은 `test.beforeAll`에서 `helpers.resetWorkspace()`를 불러 시드 상태에서 시작한다(리셋 1회 ≈ 3.3–3.7초).
...
| 항목 | 값 |
| --- | --- |
| 스펙 파일 | 7 (`build` · `documents` · `jobs` · `profiles` · `render-isolation` · `schema` · `source-review`) |
| test() | 19 |
| 통과 / 실패 / 건너뜀 / flaky | **19 / 0 / 0 / 0** |
| 소요 | Playwright 보고 2.2분 (벽시계 약 2분 10초, 시드·서버 기동·리셋 7회 포함) |
| 안정성 | 리셋 도입 뒤 같은 명령을 연속 3회 실행: 19/19 · 19/19 · 19/19 (2.1m · 2.1m · 2.2m), 재시도 0 |
| v2 회귀 | `npm run test:v2` 3 passed (19.2s; graph.spec 1 · workbench.spec 2, 포트 8021) |
...
[WebServer] [e2e-control] generation 1 · workspace kg-v3-e2e-1d92o_af · main 8031 · render 8032 · control 18031

Running 19 tests using 1 worker

[WebServer] [e2e-control] reset → generation 2 · workspace kg-v3-e2e-aolk_bow · 3297 ms
  ✓   1 v3/build.spec.ts:91:1 › 문서 인계 · 제외 사유 · 스키마 · 출력 Header 편집 · 미리보기/원본 보기 · CSV/XLSX/SQLite · manifest · 새 빌드 (6.5s)
[WebServer] [e2e-control] reset → generation 3 · workspace kg-v3-e2e-71t71u3d · 3277 ms
  ✓   2 v3/documents.spec.ts:34:1 › 쉘 · 문서 표 · 상태 칩 · 필터 · 정렬 (2.8s)
  ✓   3 v3/documents.spec.ts:140:1 › 문서 등록 대화상자 · 상세 드로어(파일 보기·추출 결과·적용 프로파일·연결 스키마) · 잠긴 문서 (7.1s)
  ✓   4 v3/documents.spec.ts:363:1 › 새 snapshot(변경 감지) · Snapshot 이력 · 다중 선택 → 데이터 빌드 (4.1s)
[WebServer] [e2e-control] reset → generation 4 · workspace kg-v3-e2e-9h4izx2j · 3536 ms
  ✓   5 v3/jobs.spec.ts:72:1 › 요약 카드 · 신규 양식/매핑 검수/파싱 실패 큐 · 렌더 서버 상태 · 작업 목록 (3.7s)
  ✓   6 v3/jobs.spec.ts:265:1 › 매핑 검수 전체 승인 → 문서 정상 · 큐 0 · 묶음 처리 작업 행 (2.5s)
  ✓   7 v3/jobs.spec.ts:305:1 › 새 snapshot(변경 감지) 큐 → 동일 승계 전체 승인 → 정상 · 변경 감지 0 (3.6s)
  ✓   8 v3/jobs.spec.ts:378:1 › 실패한 등록 작업 행(오류 문구 · 이동 → 문서 드로어) · 재파싱이 도는 동안 JobBar '진행 중 작업' (17.4s)
[WebServer] [e2e-control] reset → generation 5 · workspace kg-v3-e2e-2bt_8548 · 3336 ms
  ✓   9 v3/profiles.spec.ts:64:1 › 목록 · 상태 필터 · 상세 탭(기본 정보 · 규칙 · 필드 매핑 · JSON · 변경 이력) (7.2s)
  ✓  10 v3/profiles.spec.ts:310:1 › 외부 Profile Import: v1 파싱 템플릿 → 형식 판별 · canonical 미리보기 · 경고 → 저장 → 초안 프로파일 (2.7s)
  ✓  11 v3/profiles.spec.ts:439:1 › 테스트 탭 → Source Review(테스트 모드) · 재파싱(fill) 작업 토스트 (6.1s)
[WebServer] [e2e-control] reset → generation 6 · workspace kg-v3-e2e-a2i4zc7n · 3591 ms
[timing] first render (open drawer → cells visible, 공정 기록): 1067 ms
[timing] first render polling round-trips (202 count): 1 responses
[timing] cached re-access (close → reopen drawer, in-memory client cache): 174 ms
[timing] cached re-access (after reload → open drawer → cells visible, render-server cache): 220 ms
[timing] cached window GET (200): 18 ms
[timing] cached window GET with If-None-Match (304): 19 ms
[timing] first render (image sheet 첨부 → cells visible): 986 ms
[timing] render asset GET (200 image/png): 10 ms
  ✓  12 v3/render-isolation.spec.ts:72:1 › 첫 렌더(별도 렌더 서버) · 캐시 재접근 < 1초 · ETag/304 · Cache-Control · 이미지 asset (5.0s)
[timing] register heavy document (2000x90): 1246 ms
...
[timing] navigate 문서 → 파싱 스키마 during render: 391 ms
[timing] heavy sheet render (202 → 200, 2000x90): 2183 ms
[timing] heavy sheet tail window GET (cached): 30 ms
  ✓  13 v3/render-isolation.spec.ts:233:1 › 격리: 큰 시트를 렌더하는 동안 문서 목록 API < 500ms · 화면 이동 가능 (6.1s)
...
[timing] summary
  first render (open drawer → cells visible, 공정 기록): 1067 ms
  first render polling round-trips (202 count): 1 responses
  cached re-access (close → reopen drawer, in-memory client cache): 174 ms
  cached re-access (after reload → open drawer → cells visible, render-server cache): 220 ms
  cached window GET (200): 18 ms
  cached window GET with If-None-Match (304): 19 ms
  first render (image sheet 첨부 → cells visible): 986 ms
  render asset GET (200 image/png): 10 ms
  register heavy document (2000x90): 1246 ms
  ...
  navigate 문서 → 파싱 스키마 during render: 391 ms
  heavy sheet render (202 → 200, 2000x90): 2183 ms
  heavy sheet tail window GET (cached): 30 ms
  ...
  ✓  14 v3/render-isolation.spec.ts:331:1 › 문서 목록 API 지연 기준선(10회) (1.1s)
[WebServer] [e2e-control] reset → generation 7 · workspace kg-v3-e2e-mgv4pr45 · 3536 ms
  ✓  15 v3/schema.spec.ts:42:1 › 스키마 목록 · 상세 헤더 · 트리/그래프 토글 · 필드 상세 · 사용 프로파일/연관 문서 추적 · 필드에서 Source Review (6.6s)
  ✓  16 v3/schema.spec.ts:347:1 › 필드 편집(alias 추가 → PATCH → 새 리비전) · 변경 이력 (3.0s)
[WebServer] [e2e-control] reset → generation 8 · workspace kg-v3-e2e-lqkaake6 · 3608 ms
  ✓  17 v3/source-review.spec.ts:85:1 › 검수 화면: 컨텍스트 · 시트/규칙 목록 · 실제 셀 · overlay · 확대 · 셀 이동 → 드래그 재지정 · 승인(요청 1회) · 모두 승인 · 이력/복원 · 반려 · 역방향 조회 (8.3s)
  ✓  18 v3/source-review.spec.ts:385:1 › 창 요청: 60행·26열을 넘는 문서를 스크롤하면 뷰어가 두 번째 창(A61:… / AA…)을 요청해 그린다 (2.7s)
  ✓  19 v3/source-review.spec.ts:436:1 › 잠긴 문서: 드로어 파일 보기는 Snapshot 없음 안내, 등록 뒤 잠긴 원본은 뷰어 안에서만 DRM 실패 + 다시 시도 — 나머지 화면은 정상 (3.1s)
...
| 측정 | 값 | 기준 |
| --- | --- | --- |
| 첫 렌더(드로어 열기 → 셀 표시, 공정 기록) | 1067 ms | 202 폴링 1회 |
| 첫 렌더 202 응답 수 | 1 | — |
| 캐시 재접근(드로어 닫고 다시 열기, 클라이언트 메모리 캐시) | 174 ms | < 1초 (네트워크 요청 0) |
| 캐시 재접근(새로고침 뒤 드로어 → 셀 표시, 렌더 서버 캐시) | 220 ms | < 1초 (200만, 202 없음) |
| 캐시된 창 GET (200) | 18 ms | — |
| 캐시된 창 GET + If-None-Match (304) | 19 ms | ETag 일치 → 304, `Cache-Control: private, no-cache` |
| 첫 렌더(이미지 시트 첨부 → 셀 표시) | 986 ms | 이미지는 별도 asset |
| 렌더 asset GET (200 image/png) | 10 ms | — |
| 큰 문서 등록(2000×90 셀, wait=30 작업) | 1246 ms | — |
| 캐시 안 된 큰 시트 렌더 요청 → 202 | 11 ms | 절대 렌더 완료를 기다리지 않음 |
| 렌더 중 문서 목록 API (5회 최대) | 8 ms | < 500 ms |
| 렌더 중 문서 목록 API (5회 평균) | 7 ms | — |
| 렌더 중 화면 이동 문서 → 파싱 스키마 | 391 ms | 막히지 않음 |
| 큰 시트 렌더 완료(202 → 200, 2000×90) | 2183 ms | truncated false (§5 상한 200,000셀 미만) |
| 큰 시트 마지막 창 GET(캐시) | 30 ms | — |
| 문서 목록 API 기준선(10회 평균 / 최대 / 최소) | 7 / 9 / 6 ms | — |

다른 스펙에서 관찰한 값(단언 대상은 아니며 참고용):

| 측정 | 값 |
| --- | --- |
| 작업 공간 리셋(시드 + 렌더/메인 서버 재기동) | 3.3–3.7초 × 7회 |
...
- `e2e/v3/helpers.ts` `resetWorkspace()` 추가, 7개 스펙에 `test.beforeAll(resetWorkspace)`.
...
1. `cd frontend && npm ci && npm run build` (dist가 없으면 메인 서버가 UI를 서빙하지 않는다). `npx vitest run`으로 22 파일/180 테스트 통과 확인.
2. 저장소 루트에서 `python3 -m pytest tests/test_v3_*.py tests/test_schema_v3.py -q -p no:cacheprovider` → 185 passed.
...
4. `mkdir -p test-results-v3 && KG_E2E_CHROMIUM_PATH=<chromium> KG_E2E_PYTHON=python3 npm run test:v3 2>&1 | tee test-results-v3/run.log`
   ...
   - 기대 결과: `19 passed (약 2.1–2.2m)`, `[timing]` 값이 §5 표 범위 안.
