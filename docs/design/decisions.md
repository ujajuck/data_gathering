...
막다른 길이 하나 있었다. 문서를 지울 수 없으니 적용 기록이 영원히 남고 → 프로파일은 폐기만 되고 →
그 프로파일을 쓰는 스키마도 못 지운다. 잘못된 폴더를 일괄 등록(§11)하면 되돌릴 방법이 아예 없었다.
§18이 "프로파일을 지울 수 있게" 문 하나를 열었지만, 적용 건이 있는 프로파일은 여전히 폐기가 유일한 출구라
그 위의 막힘은 그대로였다.

**무엇이 진실인가**로 갈랐다.

- 이 시스템의 진실은 `<ws>/data/raw`의 **원본 파일**과 `<ws>/schemas`·`<ws>/profiles`의 **정의 파일**이다(§6).
- 문서 등록 기록과 그 아래 snapshot·시트·영역·적용 건·매핑·리비전·추출 실행·추출값은 **전부 그 둘에서 다시 만들 수 있는 파생물**이다.
  원본을 남긴 채 문서를 지우고 다시 등록하면 같은 것이 다시 만들어진다. 그러니 지워도 진실이 사라지지 않는다.
- 반대로 스키마·프로파일 정의는 **그 자체가 자산**이고, 지금 남아 있는 다른 문서의 추출값이 그 정의를 근거로 삼는다.
  정의를 지우면 근거가 사라진다.

그래서 **문서는 진짜 삭제하고(§4.13), 스키마·프로파일은 지우지 않고 폐기한다(§4.2.3)**.
프로파일에는 이미 `POST /profiles/{id}/deprecate`가 있었지만 **스키마에는 비활성 경로가 없었다** —
`parsing_schema.status`에 `'deprecated'` 값은 DDL에 처음부터 있었는데 그것을 설정하는 API도 화면도 없었다.
이번에 `POST /schemas/{key}/deprecate`·`/activate`를 만들어 그 구멍을 닫았다.
기존의 조건부 삭제(`DELETE /schemas/{key}` · `DELETE /profiles/{id}`)는 "한 번도 쓰이지 않은 것을 치우는" 탈출구로
그대로 둔다 — 일상 경로는 폐기다.

**낱말은 '폐기' 하나로 통일했다.** 프로파일은 '폐기', 스키마는 '비활성화'로 부르면 같은 `status='deprecated'`를
두 이름으로 배우게 된다. 화면·계약·문서 모두 '폐기'이고 `frontend/tests/terms.test.tsx`가 '비활성화'를 금지어로 막는다.

함께 정한 것(구현이 임의로 뒤집지 않는다):

- **폐기한 스키마의 approved 프로파일은 계속 자동 적용된다.** 스키마 폐기는 "여기에 새 프로파일을 만들지 마라"는
  뜻이지 "이 스키마로 들어오는 문서를 멈춰라"가 아니다. 자동 적용을 멈추려면 프로파일을 폐기한다.
- **`POST /profiles/{id}/activate`는 만들지 않는다.** 폐기한 프로파일은 `approve`로 되살아난다 — 대표 문서를 다시
  지정하는 그 행동이 곧 "다시 쓰겠다"는 선언이라 문을 하나 더 낼 이유가 없다.
- **빌드 API는 폐기 스키마의 `schema_key`를 받는다.** 폐기는 목록에서 빼는 일이지 이미 뽑은 값을 못 쓰게 하는 일이 아니다.
- **대표 문서를 지우면 그 프로파일은 초안으로 내려간다**(`reference_application_id=NULL`, `status='draft'`).
  자동 승인의 근거가 그 대표 문서였는데 사라졌으니 근거 없이 승인 상태를 유지할 수 없다. 응답 `profiles_reset[]`과
  작업 기록에 남겨 사용자가 대표 문서를 다시 지정할 수 있게 한다.
- **다중 삭제 상한은 200개**(목록 한 페이지 상한과 같다). 되돌릴 수 없는 일의 사거리를 한 번에 볼 수 있는 만큼으로 묶는다.

### 불변식은 삭제에 한해 좁혔다

`document_snapshot`·`sheet`·`source_region`·`mapping_revision`·`mapping_region`·`extracted_value`·
`extracted_value_region`·`extraction_run` 여덟 개는 BEFORE DELETE 트리거로 완전히 잠겨 있었다.
"기록은 고칠 수 없다"는 규칙을 DB가 지키게 한 것인데, 그 때문에 문서 삭제가 **DB 차원에서** 불가능했다.

불변식을 버리는 대신 **"살아 있는 문서의 기록은 고칠 수 없다"로 좁혔다**(§1.10).

- 코어 DDL에 `purge_guard(token, opened_at)` 표를 두고, 위 여덟 개의 **DELETE 거부 트리거에만**
  `WHERE NOT EXISTS (SELECT 1 FROM purge_guard)`를 붙였다.
- 서비스는 삭제 트랜잭션 안에서 가드 행을 넣고, 자식부터 지우고, **커밋 전에** 그 행을 지운다.
  커밋된 DB에 가드 행이 남는 상태는 없고, `schema/db.py`가 DB를 열 때 한 번 더 비운다.
- 실수로 친 `DELETE FROM extracted_value`는 가드 밖이라 **여전히 ABORT**다. 이것을 테스트로 고정했다.
- **`<table>_no_update` 여덟 개는 한 글자도 바꾸지 않았다.** 이번 완화는 삭제에만 적용한다 — 지우는 것과
  몰래 고치는 것은 다른 일이고, 고치는 쪽은 여전히 새 snapshot·새 리비전·새 실행으로만 표현한다.

대안으로 `document.status='deleted'` 같은 소프트 삭제도 있었지만 택하지 않았다. 지워지지 않은 행이 목록·검색·
빌드 후보·큐·카운트에서 계속 걸러져야 하고, 그 필터를 한 곳이라도 빠뜨리면 "지웠는데 남아 있다"가 된다.
되돌릴 수 없는 일을 정직하게 되돌릴 수 없게 만들고, 대신 **무엇을 지웠는지 보여 주는 쪽**을 골랐다 —
확인 대화상자가 문서 이름을 세어 보이고, 응답과 `runtime_job`(`kind='delete'`)이 문서명·`source_ref`·
snapshot/적용 건/매핑/실행/값 수·원본 삭제 여부·요청자를 남긴다. 그 작업 행의 `target_id`는 언제나 NULL이다 —
지워진 문서를 가리키면 작업 내역이 죽은 링크가 된다.
