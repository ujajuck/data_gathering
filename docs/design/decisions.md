...
- render §12는 `_render_sheet`를 렌더 서버로 옮기라 했지만 경로를 받는 렌더러는 DRM Reader 격리("임시 해제 XLSX를 만들지 않는다", [drm-viewer-render-architecture.md](drm-viewer-render-architecture.md))를 깬다. 렌더는 Reader 계약의 `render` 연산(스트림)이며 `XlsxReader`가 openpyxl로 구현한다. DRM Reader는 자체 뷰포트로 구현하거나 `RENDER_UNSUPPORTED`를 낸다.
...
- **호환 껍데기를 두지 않았다.** 구 경로 re-export, 구 환경변수(`KG_V3_*`·`KG_V2_*`) 폴백,
  ...
  옛 접두 환경변수가 설정돼 있으면 시작할 때 stderr로 한 번 알린다(옛 이름만 있으면 접근 토큰이 소리 없이 꺼진다).
