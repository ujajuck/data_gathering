...
openpyxl 3.1.5이며 기존 라이브러리의 경고 4개가 있었다. 브라우저의 로컬 서버 연결은
`net::ERR_BLOCKED_BY_CLIENT`로 차단되어 실제 클릭·드래그 및 시각 검증은 수행하지 못했다.
이 제한과 실제 DRM 제품 미제공 때문에 native 원본 충실도를 실환경에서 검증했다는 주장은 하지 않는다.
