...
이 문서는 [DB 스키마 설계](db-schema-v2.md)의 목표 UI/API 계약이다.
2026-09-09에 React v2 화면과 핵심 실행 경로를 구현했다. 아래의 전체 목표 기능과 현재 구현 범위를 구분하며,
실제 경로·화면·제한은 [런타임 안내](db-schema-v2-runtime.md)를 따른다.

사용자 검토 이후 **기존 제품명·파란색·채움형 탭과 원래 순서**를 복원했다.
실제 탭은 파일 분석/개념 탐색/원본 데이터/통합 DB/템플릿 관리다.
기능 복원 상태와 전처리·이력 정책은 [복원 결정 기록](v2-restoration.md)에 명시한다.
