...
- extract: effective_spec 목록 → 'group'/'values' 이벤트 스트림(리스트/행렬/병합/stop/결합/업무키 +
  regex·앵커·relative.anchor·relations·split_delimiter).
...
from .db import Problem, dump, norm
...
from .spec import address, decimal, decimal_text, typed
...
def region(ws, r, c):
    ...
    if not hasattr(ws, "_merge_rows"):
        ws._merge_rows = {}
    if r not in ws._merge_rows:
        if len(ws._merge_rows) >= 128:
            ws._merge_rows.pop(next(iter(ws._merge_rows)))
        spans = sorted(
            (m for m in ws.merged_cells.ranges if m.min_row <= r <= m.max_row),
            key=lambda m: m.min_col,
        )
        ws._merge_rows[r] = ([m.min_col for m in spans], spans)
    starts, spans = ws._merge_rows[r]
