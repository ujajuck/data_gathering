...
`filewatch.FileEventWatcher`(폴링 + 안정화 판정)로 변경을 찾고,
등록은 `Service.register_documents`(작업 1개: describe 1회 → snapshot → auto_apply → 추출·발행)로 보낸다.
...
from .filewatch import FileEventWatcher, IngestEvent
