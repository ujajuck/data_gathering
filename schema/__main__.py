"""python -m schema [serve] --ws <ws> --port 8031 [--host 127.0.0.1]
...
def build_parser():
    parser = argparse.ArgumentParser(prog="python -m schema", description="Semantic Excel Integration")
    ...
    profile = sub.add_parser("import-profile", help="파싱 프로파일 정의 가져오기(3.0/이전 템플릿/generic)")
...
def normalize_argv(argv):
    # 서브커맨드 없이 --ws/--port만 주던 호출(python -m schema --ws X --port N)은 serve로 해석한다.
    argv = list(argv)
...
def run_build(args):
    ...
    try:
        ...
        raise Problem("NOT_AVAILABLE", "빌드 모듈(schema.build)이 아직 없습니다.", 501) from None
...
def main(argv=None):
    ...
    if args.command == "watch":
        try:
            ...
            print("watch는 아직 제공되지 않습니다(schema.watch 없음).", file=sys.stderr)
            return 2
